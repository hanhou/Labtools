...
% need to account for only training sets of data in which no spike data is
% collected. Did this because the matrix is too big to do transpose
% operations on, so there is no need to keep is. This only applies if there
% is no spike data in the data set. ----- Tunde 3/26/10


if  (size(epoch, 1) == 5)     
    epoch_sum = sum(epoch, 2);
    num_spikes = epoch_sum(1) + epoch_sum(3) + epoch_sum(4) + epoch_sum(5);
    if num_spikes == 0 % this this is a training set and the spike data can either be thrown out or truncated as necessary
        epoch = epoch(1:2, :);
    end
end
