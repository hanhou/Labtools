...
if cbv==0
   ...
   if ~isempty(raster)
		...
		for sacnum=1:length(y)
            ...
            spikes=raster(trl,find(raster(trl,:)~=0)) - sacst(sacnum);
   ...
   if dispmode
      ...
      fileid = 'Z:\data\TEMPO\STSAplots.mat';
		...
		if exist(fileid, 'file') ~= 0
         ...
         raw = [raw; msacta*1000];
      	boxcar = [boxcar; sacdist*1000];
      	convolved = [convolved; z*1000];  
         timebins = msactatimes;
         
         num_plots = num_plots + 1;
         sites{num_plots} = filename;               
      	save (fileid, 'raw', 'boxcar', 'convolved', 'sites', 'num_plots', 'timebins');     
	end
   
   %%% 2nd figure
   
      figure('Units','inches','Position',[6 .3 5 9])
		set(gcf,'Number','off','Name',figid)
		set(gcf,'DefaultAxesFontName','Palatino','DefaultAxesFontSize',8)
      ...
      c=get(gcf,'children');
      c=sort(c);
		uicontrol('Style','pushbutton','Position',[3.7 8.6 .8 .28],'String', ....
      	'CLOSE','Callback','msactamaker(1);','FontWeight','bold')
      ...
      text('Position',[0 .3],'String',['Maximum at ', num2str(tcrest),' ms:  Mean:', num2str(meancrest), ' spikes/s'])
		text('Position',[0 .15],'String',['Baseline (-25 ms to 25 ms):  Mean:', num2str(meanbase), ' spikes/s'])
