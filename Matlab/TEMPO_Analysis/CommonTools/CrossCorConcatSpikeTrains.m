...
loadmsac = 1;
    if (loadmsac == 1)
        ...
        eval(['load ' PATHOUT FILEOUT ' minimum_segment_length mini_segment_start_times mini_segment_end_times -MAT'])
...
%mini_segment_start_times{2} = 0:100:2900;
%mini_segment_end_times{2} = 100 + mini_segment_start_times{2};

%mini_segment_start_times{2} = 0;
%mini_segment_end_times{2} = 3000;
%mini_segment_length = 1 + max(mini_segment_end_times{2} - mini_segment_start_times{2});

%analyze by unique condition
for cond = 2: 2 %num_conditions
    cond
%    for segment = 1: length(mini_segment_start_times{2})
...
%    segment
    ...
    spikes = zeros(num_reps, (2 * length(mini_segment_start_times{2}) - 1) *mini_segment_length  );    
    spikes2 = spikes;
    ...
    for trial = 1:num_reps
        ...
        stop_spike_bin = StartEventBin(reps(trial)) + StartOffset + AnalInterval;
        start_spikebin = StartEventBin(reps(trial)) + mini_segment_start_times{2}(segment);
        stop_spike_bin = StartEventBin(reps(trial)) + mini_segment_end_times{2}(segment);


        switch neural_db1
            case SPIKE_DB
                spikes(trial, (2 * (segment - 1) )*mini_segment_length + 1: (2 * (segment - 1) + 1)*mini_segment_length ) = conv(spike_density_filter, data.spike_data(SpikeChan, start_spikebin: stop_spike_bin, reps(trial))  ); 
            case LFP_DB
        ...
        switch neural_db2
            case SPIKE_DB
                 spikes2(trial, (2 * (segment - 1) )*mini_segment_length + 1: (2 * (segment - 1) + 1)*mini_segment_length ) = conv(spike_density_filter, data.spike_data(SpikeChan2, start_spikebin: stop_spike_bin, reps(trial))  ); 
           case LFP_DB
...
%    else
%        histo(cond,:) = ((segment - 1)* histo(cond,:) + mean(cross_corr_matrix(logical(crosses),:) ) /segment );         
        %    end
        histo(cond,:) = (( 1)* histo(cond,:) + mean(cross_corr_matrix(logical(crosses),:) ) / 1 );         
