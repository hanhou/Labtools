function [pars,freq] = gaborfit(means,raw,fixed_param_flags,fixed_param_values)
...
A=[]; B=[]; Aeq=[]; Beq=[]; NONLCON=[];
LB=[0;0;min_disp;0.1;0.9*freq;-2*pi];
UB=[1.5*max_val;1.35*(max_val - min_val);max_disp;2*(max_disp - min_disp);1.1*freq;2*pi];
