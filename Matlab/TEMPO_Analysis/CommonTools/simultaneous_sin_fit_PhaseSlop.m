function [z, J] = simultaneous_sin_fit_PhaseSlop(q, x)
...
for i=2:num_fits
   ...
   %[z(sum(trial_sizes(1:num_fits-1))+1:sum(trial_sizes(1:num_fits))) J_temp{i}] = sin_exp_func(x(sum(trial_sizes(1:num_fits-1))+1:sum(trial_sizes(1:num_fits))), q_short);
...
if nargout > 1
    ...
    for i=2:num_fits
        ...
        J(start+1:stop, index+1:index+3) = J_temp{i}(:, 4:6);
        index = index + 4;
