...
for i=.6:.1:1.4
    ...
    for j=0:10:360
        ...
        q(3) = j * pi/180;
        index = 6;
        err_grid(count_i,count_j) = multi_sinerr_PhaseSlop(q);
        count_j = count_j+1;
    end
    count_i = count_i+1;
end
...
for i=2:length(Data)
    LB = [LB; 0; min_val_array(i); -5; -0.3927];
    UB = [UB; (max_val_array(i)-min_val_array(i))*2; max_val_array(i); 5; 0.3927];
end
...
for j=1:N_reps
    ...
    testpars{j} = lsqcurvefit('simultaneous_sin_fit_PhaseSlop', temp_q, RawX, RawY, LB, UB, OPTIONS);
    err(j) = multi_sinerr_PhaseSlop(testpars{j});
    j
end
