...
function PSTH(data, SpikeChan, start_code, stop_code, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol);
   ...
   total_spike_bins = 3000;   % should be same for all trials
   bin_width = 10;  % in ms
 ...
 % 	  	hist_data(1,:) = data.spike_data(SpikeChan, start_spikebin : total_spike_bins, reps(1)); 
 		hist_data(1,:) = data.spike_data(SpikeChan, StartEventBin(1) : StartEventBin(1) + AnalInterval, reps(1)); 
 
		for trial = 2: num_reps
      	 start_bin = StartEventBin(reps(trial)) + StartOffset;
%         hist_data = hist_data +  data.spike_data(SpikeChan, StimStart(reps(trial)) - 500: (StimStart(reps(trial)) + 2499), reps(trial));   
          hist_data = hist_data +  data.spike_data(SpikeChan, start_bin: start_bin + AnalInterval, reps(trial));   
...
%      end
      ...
      scale(cond) = max(hist_data2(cond,floor(500/bin_width): floor(2000/bin_width)  )  );
   ...
   spontaneous_PSTH = hist_data2(unique_conds(:,1) == data.one_time_params(NULL_VALUE),:)/moving_max;
 ...
 %  marker_time = find(event_data{curr_condition, num_reps{curr_condition}} == marker_code);
   ...
   if composite == 1
      ...
      stat_PSTH_out = (stat_PSTH_out*num_files + stationary_PSTH)/(num_files + 1);      
      mov_PSTH_out = (mov_PSTH_out*num_files + moving_PSTH)/ (num_files + 1);
      spont_PSTH_out = (spont_PSTH_out*num_files + spontaneous_PSTH)/(num_files + 1);
      num_files = num_files + 1;
      
      pref_disp = [pref_disp; pref_hdisp]; 
            
      save (fileid, 'stat_PSTH_out', 'mov_PSTH_out', 'num_files', 'pref_disp', 'spont_PSTH_out');
