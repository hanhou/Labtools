function pars = sin_exp_fit_model(means,raw)
...
% do a quick search for a good starting phase and frequency
...
[val2 ind2] = min(val);
q(3) = phase_search(ind(ind2));
q(2) = freq_search(ind2) * pi/180;
...
for j=1:N_reps
    ...
    err(j) = sin_exp_err_model(testpars{j});
    j
end
...
%A=[]; B=[]; Aeq=[]; Beq=[]; NONLCON=[];
%LB=[ 0; .6; -2*pi; min_val; -5];
%UB=[ (max_val-min_val)*2; 1.4; 2*pi; max_val; 5];

%OPTIONS = OPTIMSET('fmincon');
%OPTIONS = OPTIMSET('LargeScale', 'off', 'LevenbergMarquardt', 'on', 'MaxIter', 5000, 'Display', 'off', 'MaxFunEvals', 2500, 'Diagnostics', 'on');
...
%    temp_q = q' .* rand_factor;
%    pause(.03);
%    testpars{j} = fmincon('sin_exp_err_model',temp_q,A,B,Aeq,Beq,LB,UB, NONLCON, OPTIONS);
%    err(j) = sin_exp_err_model(testpars{j});
%end
%now find best fit and return the parameters
%[min_err min_indx] = min(err);
