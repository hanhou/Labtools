...
%We only need addpath once and addpath will take a long time.
%Here we setup a global variable g_path_defs to signal that we have already
%add all path. Elapsed time for one analysis changed from 51s to 3.6s. -HyungGoo and Johnny 9/28/2012
if isempty(g_path_defs) || g_path_defs ~= 7
