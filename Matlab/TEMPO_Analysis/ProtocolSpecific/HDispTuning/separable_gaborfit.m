function [pars] = separable_gaborfit(means,raw)
...
q(1) = mean(Data1(:,2));
q(2) = 2;  %amplitude/baserate ratio, using GABORFUNC_ALT.M
q(3) = 0.0;
q(4) = 0.5; % 
q(5) = 0.5; % 
q(6) = -1.5;
...
A=[]; B=[]; Aeq=[]; Beq=[]; NONLCON=[];
LB=[0;0.01;min_disp_1;0.1;0.3*freq;-2*pi;0];
UB=[1.5*max_val_1;100;max_disp_1;2*(max_disp_1 - min_disp_1);3.0*freq;2*pi;1.5*max_val_2];
...
for j=1:N_reps
    ...
    testpars{j} = fmincon('separable_gaborerr',temp_q,A,B,Aeq,Beq,LB,UB, NONLCON, OPTIONS);
    err(j) = separable_gaborerr(testpars{j});
end
%err
