...
%--	GCD, 5/23/01
%-----------------------------------------------------------------------------------------------------------------------
function HDispFit_MultiSpeed(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
%**********************************************************************************************
[pars] = paired_gaborfit(means,raw);

subplot(2, 2, 4);
hold on;
pars1 = pars(1:6); %parameters for the 1st Gabor
gabor1 = gaborfunc(x_interp, pars1);
gabor1(gabor1 < 0) = 0;
...
gabor2_coarse = gaborfunc(means{2}(:,1), pars2);
...
indep_params = 12;  paired_params = 8;
...
xpos = -20; ypos = 110;
font_size = 8;  bump_size = 9;
line = sprintf('Base Rate 1: %8.4f ', pars(1) );
text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
line = sprintf('Amplitude 1: %8.4f ', pars(2));
text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
line = sprintf('Gauss Ctr: %8.4f ', pars(3));
text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
line = sprintf('Gauss  SD: %8.4f ', pars(4));
...
line = sprintf('Sin Phase: %8.4f ', pars(6));
text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
line = sprintf('Base Rate 2: %8.4f ', pars(7) );
text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
line = sprintf('Amplitude 2: %8.4f ', pars(8));
text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
...
outfile = [BASE_PATH 'ProtocolSpecific\HDispTuning\MultiSpeed_GaborFit.dat'];
printflag = 0;
