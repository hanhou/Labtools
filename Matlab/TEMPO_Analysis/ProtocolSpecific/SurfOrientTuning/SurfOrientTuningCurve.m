...
%print grad metrics
outfile = [PATHOUT 'Con_MD_TDI_12.01.04.dat'];
fCon = fopen(outfile, 'a');
outfile = [PATHOUT 'Vel_MD_TDI_12.01.04.dat'];
fVel = fopen(outfile, 'a');
outfile = [PATHOUT 'Disp_MD_TDI_12.01.04.dat'];
fDisp = fopen(outfile, 'a');
outfile = [PATHOUT 'Txt_MD_TDI_12.01.04.dat'];
...
%             vector_yavg(stim_count,mdepth_count) = sum_y/mag;
            ...
            peak_resp(stim_count,mdepth_count) = spk_max.y;
...
%            YLim([0 yl(2)])	% set the lower limit of the Y axis to zero
            ...
            if printcurves == 1
                ...
                FILEOUT = [FILE(1:filesize) 'surf_curveMU'];
        ...
        %calculate average peak response for each mean depth
        peak_mean(stim_count) = mean(peak_resp(stim_count, :));
        ...
        line = sprintf('\t%3.2f\t%1.5f\t%1.5f\t%3.2f\t%1.5f\t%1.5f\t%1.5f\t%3.2f', unique_stim(stim_count), avgTDI_adj(stim_count), avgTDIverg_adj(stim_count), spk_max.x, peak_mean(stim_count), verg_max.x, p(1), verg_p(1), null_y(1));
        data_string = strcat(data_string, line);
...
%     data_string = strcat(line, data_string);
    ...
    %print grad metrics
    outfile = [PATHOUT 'MU_Surf_metrics_12.19.04.dat'];
    printflag = 0;
    ...
    if (printflag)
        ...
        fprintf(fid, 'File\tApSize\tX\tY\tEcc\tPrefDir\tStimType\tTDI\tTDIverg\tPrefTiltNeuron\tPeakMean\tPrefTiltVerg\tPSpike\tPVerg\tSpont\tStimType1\tTDI1\tTDIverg1\tPrefTiltNeuron1\tPeakMean1\tPrefTiltVerg1\tPSpike1\tPVerg1\tSpont1\tStimType2\tTDI2\tTDIverg2\tPrefTiltNeuron2\tPeakMean2\tPrefTiltVerg2\tPSpike2\tPVerg2\tSpont2\tStimType3\tTDI3\tTDIverg3\tPrefTiltNeuron3\tPeakMean3\tPrefTiltVerg3\tPSpike3\tPVerg3\tSpont3\n');
        printflag = 0;
    end
