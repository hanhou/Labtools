...
if TURN_ON_MOVIE
    ...
    %clear M;
    m_index = 1;
    %aviobj = avifile('C:/Temp/cu_cell.avi');
end
if TURN_ON_AUDIO
    spike_sound = [];
end
...
%for i = 1:2
    ...
    if TURN_ON_AUDIO
        spike_sound = [spike_sound, 8*interp(spikes, 12)];
    end
    ...
    if TURN_ON_MOVIE
        disp('Writing avi...');
        fname = ['C:/Temp/cu_cell', num2str(i), '.avi'];
        movie2avi(M, fname, 'COMPRESSION', 'Cinepak');
        %aviobj = close(aviobj);
        disp('Avi finished');
        m_index = 1;
    end
...
if TURN_ON_AUDIO
    disp('Writing audio...');
    wavwrite(spike_sound, 'C:/Temp/cu_sound.wav');
    disp('Audio finished');
end
