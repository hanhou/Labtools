...
% datadir = 'Z:\Data\Tempo\Spumoni\Analysis\Unfolded-Psychophysics\';
% datadir = 'Z:\Data\Tempo\Baskin\Analysis\Unfolded-Psychophysics\LongDurCells\';
% datadir = 'Z:\Data\Tempo\Baskin\Analysis\Unfolded-Psychophysics\ShortDurCells\';
datadir = 'Z:\Data\Tempo\Baskin\Analysis\Unfolded-Psychophysics\ShortDurDelayCells\';
temp = ls(datadir);
temp = temp(3:end,:);
d = [];
for i = 1:size(temp,1)
...
coher = 2*[0 -2 -4 -8 -16 2 4 8 16];
cum_psychdata = [];
for i = 1:size(d,1)
    c = importdata(sprintf('%s%s',datadir,d(i,:)));
    cum_psychdata(:,:,end+1) = c(2:4,1:9); %indices of pct correct at each cue_dir
end
plot_colors = {'bs','ro','g>'};
plot_lines = {'b','r','g'};
...
for i = 1:3
    ...
    fit_x = [min(coher)-3:0.1:max(coher)+3];
...
axis('tight'); ylim([0 1]); axis('manual');
plot(xlim,[0.5 0.5],'k:')
for i = 1:3
    h(i) = plot(-1,-1,plot_linecolor{i});
end
l = legend(h,'Null','Neutral','Pref','Location','SouthEast');
set(l,'box','off')
xlabel('Signed Coherence');
ylabel('Fraction Preferred Choices');
title(sprintf('(n=%d)',size(d,1)));

temp = [coher' mean_psychdata'; coher' (std_psychdata./sqrt(size(d,1)))'; fit_x' fit_y'];
save('Z:\LabTools\Matlab\TEMPO_Analysis\ProtocolSpecific\CuedDirectionDiscrim\group_unfoldedpsych_shortdur_delay.txt', '-ascii', 'temp')
