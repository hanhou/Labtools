% datadir = 'Z:\Data\Tempo\Baskin\Analysis\CuedDirecDiscrim_RT\LongDurCells\'
% datadir = 'Z:\Data\Tempo\Baskin\Analysis\CuedDirecDiscrim_RT\ShortDurCells\'
datadir = 'Z:\Data\Tempo\Baskin\Analysis\CuedDirecDiscrim_RT\ShortDurDelayCells\'
d = ls(datadir);
d = d(3:end,:);
...
for i = 1:size(d,1)
    if ~ismember(d(i,4:11), data_exclude)
        ...
        cum_rt(:,:,end+1) = c(2:6,1:5); %indices of mean rt at each coherence at each of 4+1(avg) conditions
    end
end
cum_rt = cum_rt(:,:,2:end); %not sure why it's making a matrix of zeros
...
y = reshape(cum_rt(1:3,:,:),n_cells*5*3,1);
cells = reshape(repmat(1:n_cells,15,1),n_cells*5*3,1);
f1 = repmat([1:3]', n_cells*5,1); %cuedir
f2 = reshape(repmat(1:5,3,n_cells),n_cells*5*3,1);
...
save('Z:\LabTools\Matlab\TEMPO_Analysis\ProtocolSpecific\CuedDirectionDiscrim\group_rt_shortdurdelay.txt', '-ascii', 'temp')