...
for j = 1:length(unique_x_ctr)
    ...
    [monkey_alpha(j) monkey_beta(j)]= constrained_weibull_fit(fit_data, FIT_THRESH_UB, FIT_SLOPE_UB);
    
    fit_x = unique_coherence(1):0.1: FIT_THRESH_UB;
...
for j = 1:length(unique_x_ctr)
    for b = 1 : bootstp_num
        ...
        bootthr(b,j) = constrained_weibull_fit([unique_coherence psycho_correct_boot(b,:)' repetition.*ones(length(unique_coherence),1)], FIT_THRESH_UB, FIT_SLOPE_UB);
