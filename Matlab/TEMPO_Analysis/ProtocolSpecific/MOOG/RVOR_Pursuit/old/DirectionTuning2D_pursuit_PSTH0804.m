...
%--	Yong, 6/27/03  Modified by Katsu, 2/1/06 08/03/06
%-----------------------------------------------------------------------------------------------------------------------
...
%get the column of values for azimuth and elevation and fp_rotate
...
%     for j=1:length(unique_condition_num)
      for j=1:length(unique_elevation)+1
       axes('position',[(0.01+0.09*j) (1.0-0.2*k)  0.085 0.085])
         ...
         if (j < 8 )                                 % temporarilly line output figure with contour one, so that the middle panel corresponds to 90 deg,                             
            bar( x_time,count_y{1,j,k}(1,:) );    % j=9=1 == 0 degrees
        else
            bar( x_time,count_y{1,1,k}(1,:) ); 
        end                                
