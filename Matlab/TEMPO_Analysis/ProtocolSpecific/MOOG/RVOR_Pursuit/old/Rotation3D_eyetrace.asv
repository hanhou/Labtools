...
for i=1:8
   ...
   %FP_rotate=0 means head-fixed now select 0
    ...
    figure(3);subplot(2,4,i);plot(TimeLabel,Position_H1(:,1), 'b', TimeLabel,Position_H2(:,1),'r',TimeLabel,Position_H0(:,1),'g'); %,TimeLabel,Position_TH2,'c'
    %legend('Rotation','Pursuit only','Target // Rotation','Target // Pursuit only',2); 
    hold on;plot(TimeLabel,Position_H1(:,2:end), 'b', TimeLabel,Position_H2(:,2:end),'r',TimeLabel,Position_H0(:,2:end),'g'); %,TimeLabel,Position_TH2(:,2:end),'c'
    xlabel('Time (ms)');ylabel('Horizontal Position');title([FILE,'/',Eye_Select,'/',num2str((i-1)*45)]);axis([1000 3000 -12 12]);
 
    figure(4);subplot(2,4,i);plot(TimeLabel,Position_V1(:,1), 'b', TimeLabel,Position_V2(:,1),'r',TimeLabel,Position_V0(:,1),'g'); %,TimeLabel,Position_TV2,'c'
    %legend('Rotation','Pursuit only','Target // Rotation','Target // Pursuit only',2); 
    hold on;plot(TimeLabel,Position_V1(:,2:end), 'b', TimeLabel,Position_V2(:,2:end),'r',TimeLabel,Position_V0(:,2:end),'g'); %,TimeLabel,Position_TV2(:,2:end),'c'
    xlabel('Time (ms)');ylabel('Vertical Position');title([FILE,'/',Eye_Select,'/',num2str((i-1)*45)]);axis([1000 3000 -12 12]);
    
    clear Velocity_H1 Velocity_H2  Velocity_V1 Velocity_V2 Velocity_V0 Velocity_V0 ;      
    for j=1:size(Position_H1,2)
        ...
        Velocity_H0(:,j)=fderiv(Position_H0(:,j),15,SR);
        ...
        Velocity_V0(:,j)=fderiv(Position_V0(:,j),15,SR);
    ...
    figure(5);subplot(2,4,i);plot(TimeLabel,Velocity_H1(:,1), 'b', TimeLabel,Velocity_H2(:,1),'r',TimeLabel,Velocity_H0(:,1),'g');%,TimeLabel, Velocity_TH2(:,1),'c'
    %legend('Rotation','Pursuit only','Target // Rotation','Target // Pursuit only',2); 
    hold on; plot(TimeLabel,Velocity_H1(:,2:end), 'b', TimeLabel,Velocity_H2(:,2:end),'r',TimeLabel,Velocity_H0(:,2:end),'g');%,TimeLabel, Velocity_TH2(:,2:end),'c'
    xlabel('Time (ms)');ylabel(' Horizontal Velocity');title([FILE,'/',Eye_Select,'/',num2str((i-1)*45)]);axis([1000 3000 -30 30]);   
    
    figure(6);subplot(2,4,i);plot(TimeLabel,Velocity_V1(:,1), 'b', TimeLabel,Velocity_V2(:,1),'r',TimeLabel,Velocity_V0(:,1),'g');%,TimeLabel, Velocity_TV2(:,1),'c'
    %legend('Rotation','Pursuit only','Target // Rotation','Target // Pursuit only',2); 
    hold on; plot(TimeLabel,Velocity_V1(:,2:end), 'b', TimeLabel,Velocity_V2(:,2:end),'r',TimeLabel,Velocity_V0(:,2:end),'g');%TimeLabel, Velocity_TV2(:,2:end),'c'
