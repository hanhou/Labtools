...
for i = 1:length(total_trials)
    ...
    target_onset_bin(i) = floor(targ_on_diode(i)/timebin) + 1; %Using the photodiode timing instead of tempo
...
for k=1: length(unique_amplitude)
    for i=1:length(unique_heading)
        ...
        targ_raster_start = min(targ_on_diode); %align all data to this later
        targ_raster_end = max(targ_on_diode);
