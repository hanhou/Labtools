...
for i=1:8
    clear Position_H1 Position_H2  Position_V1 Position_V2   %Position_H0  Position_V0;
...
%     Position_V0(:,:)=data.eye_data(Ver,StartPoint:EndPoint,Select_Trial);%Vertical_Rotation
    ...
    clear Select_Trial; Select_Trial=find(temp_stim_type==1 & temp_fp_rotate==0 & temp_rot_elevation==(i-1)*45);%FP_ROTATE(TT_MODA)=0 Tilt=Trans 
    ...
    clear Select_Trial; Select_Trial=find(temp_stim_type==1 & temp_fp_rotate==1 & temp_rot_elevation==(i-1)*45);%FP_ROTATE=1 Tilt-Trans
...
%     clear Select_Trial; Select_Trial=find(temp_stim_type==1 & temp_fp_rotate==2 & temp_rot_elevation==(i-1)*45);%FP_ROTATE=2 Tilt only
...
%     
%     clear Select_Trial; Select_Trial=find(temp_stim_type==1 & temp_fp_rotate==3 & temp_rot_elevation==(i-1)*45);%FP_ROTATE=3 Trans only
...
%     title([Eye_Select,'/',num2str((i-1)*45)]);axis([1000 3000 -12 12]);set(gca, 'xtick', [] );
    ...
    subplot(8,1,i);plot(TimeLabel,Position_H1(:,:), 'b', TimeLabel,Position_H2(:,:),'r',TimeLabel,Position_TH1(:,1),'k');
    title(['/H/',num2str((i-1)*45)]);
    hold on;
    axis([1000 3000 -3 3]);set(gca, 'xtick', [] );
    
    figure(10);set(10,'Position', [5,5 980,650], 'name',FILE);
    subplot(8,1,i);plot(TimeLabel,Position_V1(:,:), 'b', TimeLabel,Position_V2(:,:),'r',TimeLabel,Position_TV1(:,1),'k');
    title(['/V/',num2str((i-1)*45)]);
    hold on;
    axis([1000 3000 -3 3]);set(gca, 'xtick', [] );
