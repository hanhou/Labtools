...
%-----------------------------------------------------------------------------------------------------------------------
function [training_inputs, training_outputs] = make_training_set(fun,fun_eye,Pt,P_eye,...
...
%         az_ec = unique_point_azimuth_r; el_ec = unique_point_elevation_r;
        ...
        for k=1:num_neurons
            Ptmp = Pt(k,1:num_params);
            
            % input layer does not vary amplitude as a function of gaze angle
            Ptmp(curvefit_parameter_mapping(CURVEFIT_PARAM_AMPLITUDE)) = ...
                Ptmp(curvefit_parameter_mapping(CURVEFIT_PARAM_AMPLITUDE_OFFSET));
            
            % input layer does not vary dc offset as a function of gaze angle
            Ptmp(curvefit_parameter_mapping(CURVEFIT_PARAM_DC_OFFSET)) = ...
                Ptmp(curvefit_parameter_mapping(CURVEFIT_PARAM_DC_OFFSETB));
            ...
            if m == CURVEFIT_VESTIBULAR_STIM
                ...
                training_inputs(k,beg_i:end_i) = feval( fun, Ptmp, az_ec, el_ec );
                %training_inputs(k,beg_i:end_i) = feval( fun, Ptmp, az_hc, el_hc );
            end
            
            % compute our output fit data in hc coords (or ec coords)
            training_outputs(k,beg_i:end_i) = feval( fun, Ptmp, az_hc, el_hc );
            %training_outputs(k,beg_i:end_i) = feval( fun, Ptmp, az_ec, el_ec );
            ...
            if m == CURVEFIT_OCCULAR_STIM
                ...
                training_inputs(num_neurons+2*num_eye_neurons+k,beg_i:end_i) = ...
                    feval( fun, Ptmp, az_hc, el_hc );
                %training_inputs(num_neurons+2*num_eye_neurons+k,beg_i:end_i) = ...
                %    feval( fun, Ptmp, az_ec, el_ec );
            end        
        end            
