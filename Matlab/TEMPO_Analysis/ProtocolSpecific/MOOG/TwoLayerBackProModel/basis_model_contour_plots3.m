...
function basis_model_contour_plots(unique_azimuth, unique_elevation, ...
    unique_point_azimuth, unique_point_elevation, input_ec_resp, input_vestib_resp, ...
    actual_hc_resp, expected_hc_resp)
...
max_data = max( [max(expected_hc_resp) max(actual_hc_resp) max(input_ec_resp)] );
min_data = min( [min(expected_hc_resp) min(actual_hc_resp) min(input_ec_resp)] );
...
for m=1:num_grid_elevation
    for n=1:num_grid_azimuth
        ...
        plot_ec_data(m,n) = input_ec_resp(select);
        plot_vestib_data(m,n) = input_vestib_resp(select);
...
% plot the ec data inputs
subplot(1,5,1);
[C h] = contourf(plot_azimuth,plot_elevation,plot_ec_data,data_clines);
set(gca,'ydir','reverse','xdir','reverse','xtick',x_tick,'ytick',y_tick);
axis([x_min,x_max,y_min,y_max]); caxis([min_data max_data]); 
if plot_clabels & (length(C)>0), clabel(C,h), end;    
xlabel('azimuth'); ylabel('elevation');
title('ec occular inputs');

% plot the vestib data inputs
subplot(1,5,2);
[C h] = contourf(plot_azimuth,plot_elevation,plot_vestib_data,data_clines);
set(gca,'ydir','reverse','xdir','reverse','xtick',x_tick,'ytick',y_tick);
axis([x_min,x_max,y_min,y_max]); caxis([min_data max_data]); if plot_clabels & (length(C)>0), clabel(C,h), end;    
xlabel('azimuth'); ylabel('elevation');
title('hc vestib inputs');
...
axis([x_min,x_max,y_min,y_max]); caxis([min_data max_data]); if plot_clabels & (length(C)>0), clabel(C,h), end;
xlabel('azimuth'); ylabel('elevation'); 
title('network hc/ec outputs');

% plot the first residual
% subplot(1,3,3);
% [C h] = contourf(plot_azimuth,plot_elevation,plot_residual_data);
% set(gca,'ydir','reverse','xdir','reverse','xtick',x_tick,'ytick',y_tick);
% axis([x_min,x_max,y_min,y_max]); %caxis([-range_data/2 range_data/2]); if plot_clabels & (length(C)>0), clabel(C,h), end;
% xlabel('azimuth'); ylabel('elevation');
subplot(1,5,5);
