function [p,t] = postmnmx(rng,pn,minp,maxp,tn,mint,maxt)
...
if nargin==6
  ...
  t = (tn-rng(1))/(rng(2)-rng(1));
  %t = t.*((maxt0-mint0)*oneQ) + mint0*oneQ;  
  % use this one to normalize over all inputs
  t = t.*(max(maxt0)-min(mint0)) + min(mint0);
end
