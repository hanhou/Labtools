function [pn,minp,maxp,tn,mint,maxt] = premnmx(rng,p,t)
...
%pn = (rng(2)-rng(1))*(p-minp0*oneQ)./((maxp0-minp0)*oneQ) + rng(1);
% use this one to normalize over all inputs
pn = (rng(2)-rng(1))*(p-min(minp0))./(max(maxp0)-min(minp0)) + rng(1);
...
if nargin==3
  ...
  %tn = (rng(2)-rng(1))*(t-mint0*oneQ)./((maxt0-mint0)*oneQ) + rng(1);
  % use this one to normalize over all inputs
  tn = (rng(2)-rng(1))*(t-min(mint0))./(max(maxt0)-min(mint0)) + rng(1);
end
