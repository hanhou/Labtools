...
repetitionN = floor( length(spike_rates) / (length(unique_azimuth) * length(unique_elevation)* length(unique_stim_type)*length(unique_motion_coherence)) ); % take minimum repetition
    ...
    for k=1:length(unique_stim_type)
        for i=1:length(unique_azimuth)
            ...
            if (sum(select) > 0)  % there are situations where visual/combined has >2 coherence and vestibular only has one coherence
                ...
                resp_err{c}(i,k) = std(spike_rates(select)) / sqrt(repetitionN);
                
                spike_temp = spike_rates(select);
                try
                resp_trial{c,k}(1:repetitionN,i) = spike_temp(1:repetitionN);
                catch
                    keyboard
                end
            else
                ...
                resp_trial{c,k}(1:repetitionN,i) = resp_trial{1,k}(1:repetitionN,i);
            end
        end
    end
    ...
    for k = 1: length(unique_stim_type)
       ...
       % Modulation Index
        ...
        DDI(c,k) = ( max(resp{c}(:,k))-min(resp{c}(:,k)) ) / ( max(resp{c}(:,k))-min(resp{c}(:,k))+ ...
                2 * sqrt( sum(resp_std{c}(:,k).^2 / length(unique_azimuth)))) ;    % delta / (delta + 2 * sqrt(SSE/(N-M))) = delta / (delta + 2 * sqrt(SSE/(rep-1)/M)))
...
%     end
    ...
    for c=1:length(unique_motion_coherence)
        ...
        set(errorbar(unique_azimuth_plot_heading, resp{c}(aziToHeadingSort,k), resp_err{c}(aziToHeadingSort,k), f{c,unique_stim_type(k)} ),'linewidth',2);
...
%         end
        ...
        if sum(abs(unique_azimuth_plot_heading) == 180) > 0
            ...
            [cf_, goodness, output]= fit(unique_azimuth*pi/180,resp{c}(:,k),ft_,fo_);
...
%     set(gca,'xtick',0:45:350);
    ...
    polarwitherrorbar([unique_azimuth/180*pi; unique_azimuth(1)/180*pi], [resp{c}(:,k); resp{c}(1,k)],[resp_err{c}(:,k) ;resp_err{c}(1,k)],f{c,unique_stim_type(k)}(1));
    hold on;
...
text(80, length(unique_stim_type)*length(unique_motion_coherence)+2, sprintf('rep = %g',repetitionN));
...
for k=1:length(unique_stim_type)  
    for c=1:length(unique_motion_coherence)     
        ...
        text(140,length(unique_stim_type)*length(unique_motion_coherence)-(count-1), num2str(widGauss(c,k)) );
    end
end
axis off;
