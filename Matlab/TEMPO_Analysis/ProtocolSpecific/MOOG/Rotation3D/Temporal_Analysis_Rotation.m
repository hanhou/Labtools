...
for k=1:length(unique_condition_num)
    for j=1:length(unique_elevation)
        for i=1: length(unique_azimuth)
            select = logical( (temp_azimuth==unique_azimuth(i)) & (temp_elevation==unique_elevation(j)) & (temp_condition_num==unique_condition_num(k)) );
...
% %---------------------------------------------------------------------------------------
% % The following is the old ANOVA code.  Commenting out in favor of Narayan's version below  -CRF
% %---------------------------------------------------------------------------------------
% 
% trials_per_rep = 26 * length(unique_condition_num) + 1; % the 1 corresponds to the null trial
...
maxrep = 0;
for k = 1:length(unique_condition_num)
    for i = 1:26 %number of unique directions           **Changed to temp_azimuth, etc. to ensure partial reps are included  -CRF 3/26/2007
         select_rep = find(temp_azimuth==unique_azimuth(plot_col(i)) & temp_elevation==unique_elevation(plot_row(i)) & temp_condition_num==unique_condition_num(k) );
...
% Fixed to include null trials ('temp' added to vectors in SELECT and spike_rates), for consistency with previous edits  -CRF 3/21/2007
for k=1: length(unique_condition_num)
