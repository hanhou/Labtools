...
%--	Katsu
%-----------------------------------------------------------------------------------------------------------------------
function Rotation_Horizontal_only(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE);
...
% outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\Horizontal_Rotaion.dat'];
% printflag = 0;
...
% fid = fopen(outfile, 'a');
% if (printflag)
%     fprintf(fid, 'FILE\t         Spon\t Vest-min\t Visual-min\t Vest-max\t W Visual-max\t VestPreDir\t VisualPreDir\t VestANOVA\t VisualANOVA\t');
% fprintf(fid, '\r\n');
% end
% fprintf(fid, '%s', buff);
% fprintf(fid, '\r\n');
