        ...
        for k=1:length(unique_stim_type)
            ...
            for j=1:length(unique_elevation)
                for i=1:length(unique_azimuth)
                    ...
                    if (sum(select) > 0)   
                        pc=pc+1;
                        repeats_temp(k,pc) = length(select_find); 
                        spike_temp = spike_rates(select);
                        raster = [];                        
                        for jj=1:length(spike_temp)
                            raster(jj,:) = spike_data(1,StartEventBin(1)+115+5000*(select_find(jj)-1) : StartEventBin(1)+2000+115+5000*(select_find(jj)-1));
                        end                       
...
%     outloop = 1;
%     insideloop = outloop+1;
%     for i = 1:channel_combos % all possible pairs
%         figure(i+1);
%         set(i+1,'Position', [5,15 980,650], 'Name', '3D Direction Tuning');
%         orient landscape;
...
%             end
%             xlabel('azimuth');
%             title( [FILE num2str(unique_stim_type(k)) '     p: ' num2str(p_1D(outloop,k)) ] );
% 
%             subplot(3,3,k+3)
...
%             subplot(3,3,k+6)
%             select_stim = logical( stim_type==unique_stim_type(k) );
%             plot(Z_Spikes(outloop,select_stim),Z_Spikes(insideloop,select_stim), 'o');
%             xmin = min([Z_Spikes(outloop,select_stim),Z_Spikes(insideloop,select_stim)]);
%             xmax = max([Z_Spikes(outloop,select_stim),Z_Spikes(insideloop,select_stim)]);
%             xlim([xmin, xmax]);
%             ylim([xmin, xmax]);
%             hold on;
%             plot([xmin,xmax], [xmin, xmax],'-');
%             title( ['noise correlation' '       '  num2str(noise_r_stim(i,k)) ]);
%         end
% 
%         if insideloop < channelcount % more to run
%             insideloop = insideloop+1;
