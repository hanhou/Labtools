...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%% 111706Katsu TEMPORALY ANOVA+Mean std  Analysis
...
% buff = sprintf('%s\t %4.2f\t  %4.4f\t   %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  %4.4f\t   %4.4f\t   %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t  %4.4f\t   %4.4f\t  ', ...
...
%    
% outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\Katsu_Ro6Cardinal.dat'];
% printflag = 0;
...
%% This is Yong's paper 270-225-180-135-90-45-0--45--90
...
VesMax=input('Enter Vestibular Max = ');
VesMin=input('Enter Vestibular Min = ');
VisMax=input('Enter Visual Max = ');
VisMin=input('Enter Visual Min = ');
CombMax=input('Enter Combined Max = ');
CombMin=input('Enter Combined Min = ');
...
for k=1: length(unique_condition_num) 
    ...
    contourf( azi_cos, ele_sin, squeeze( resp_mat_tran(k,:,:)) );  %Cosine Plot (Added by Aihua, 01-24-2007)
%     contourf( squeeze( resp_mat_tran(k,:,:)) );

if ( k==1 )           % Katsu for paper
caxis([VesMin, VesMax]);
end
if ( k==2 )
caxis([VisMin, VisMax]);
...
caxis([CombMin, CombMax]);
    ...
    x_elevation=[-1,-0.707,0,0.707,1]; %Cosine Plot (Added by Aihua,01-24-2007)
    errorbar(x_elevation,y_elevation_mean,y_elevation_ste,'k-');% Katsu for paper settle for m3c294
    view(90,90);
    set(gca, 'xtick',unique_elevation);
%     xlim([-90, 90]);
%     xlim([-1, 1]);%Cosine Plot (Added by Aihua, 01-24-2007)
    xlim([-1.1, 1.1]);%Cosine Plot (Added by Aihua, 01-24-2007)
    ylim([min(y_elevation_mean(1,:))-max(y_elevation_ste(1,:)), max(y_elevation_mean(1,:))+max(y_elevation_ste(1,:))]);axis off%----------Now disable temporaly
...
%     xlabel('Azimuth');
    ylim([min(y_azimuth_mean(1,:))-max(y_azimuth_ste(1,:)), max(y_azimuth_mean(1,:))+max(y_azimuth_ste(1,:))]);axis off%----------Now disable temporaly
...
% %outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\Rotation_Que_Labyrinthectomy_DDI_Katsu.dat'];
% printflag = 0;
% if (exist(outfile, 'file') == 0)    %file does not yet exist
%     printflag = 1;
% end
