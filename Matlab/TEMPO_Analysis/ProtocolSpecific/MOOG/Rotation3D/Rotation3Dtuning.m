...
buff = sprintf(sprint_txt, FILE, resp_mat_anova{1}(:,1), resp_mat_anova{1}(:,5));%
outfile = ['Z:\Users\Yong\MSTrotationdarkness_trailovertime.dat'];
...
% saveas(gcf,['C:\Aihua\z_TempOutputs\figures\' FILE(1:end-4) '.fig'])
% close (2);
% SaveTrials(FILE,BegTrial,EndTrial,P_anova_hor)
% SaveTrials1(FILE,BegTrial,EndTrial)
...
% %Also, write out some summary data to a cumulative summary file
% 
% Lefteye = mean(mean(data.eye_data(1,201:600,:)));
% Righteye = mean(mean(data.eye_data(3,201:600,:)));
% if Lefteye<=3 & Righteye>3
%     eyeflag = 1;
% end
% if Righteye<=3 & Lefteye>3
%     eyeflag = 2;
% end
% if Lefteye<=3 & Righteye<=3
%     eyeflag = 3;
...
%      sprint_txt = [sprint_txt, ' %4.3f'];    
% end
% buff = sprintf(sprint_txt, FILE, eyeflag );%
% outfile = ['Z:\Users\Syed\eye.dat'];
% printflag = 0;
% if (exist(outfile, 'file') == 0)    %file does not yet exist
%     printflag = 1;
% end
