% analyze slope for the 3D tuning curve in space for single cue condition
...
%	aa = dlmread('VesOnly_rot.dat','',0,1);  % load data
%    bb = aa(:,27:end);
%	cc = dlmread('p_rot.txt');
elseif analyze_protocol == 'translat';
    aaa = dlmread('MT.txt');
    aa = aaa(:,1:26);  % mean firing rates 
    bb = aaa(:,27:52);  % standard deviation
%	aa = dlmread('VesOnly_tran.dat','',0,1);  % load data
%    bb = aa(:,27:end);
...
for i = 1 : dim(1)
    ...
    ves_hori(i,:) = ves_fit{i}(3,:);
    ves_hori_norm(i,:) = ves_hori(i,:)/max(ves_hori(i,:));
...
%     vis_ele_width(i) = cc_ele_up_vis + cc_ele_down_vis;
   ...
   % Slope and Fisher information
    ...
    varience_ves( find(varience_ves)<1 ) =1; % make denominator larger than 1 to avoid unreasonable value
...
set(gca, 'xticklabel','0|90|180|270|360'); 
title('Fisher in azimuth: Visual MT, N=47');
...
set(gca, 'xticklabel','0|90|180|270|360'); 
title('Fisher in elevation: Visual MT, N=47');
