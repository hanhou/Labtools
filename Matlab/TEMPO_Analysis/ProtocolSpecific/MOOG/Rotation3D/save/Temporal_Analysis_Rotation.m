...
for j=1:26
    ...
    HT = HT(4:end-3);%Here, it turned 46 to 40 points
%     %added by Katsu
%     Hilbert{j}=HT;% HT, here, it is bad because subtranted MFR in line 216
    H_time = 1:.05:3;
    for hh = 1:length(HT)
...
%%%%%%%%%%%%%%%%% no more need 2 sentences
% F_val_vel = cell2mat(F_val_vel);
% F_val_acc = cell2mat(F_val_acc);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%comment again for Narayan's code, but Aihua 03/20/07 Katsu%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%          trial_resp_matrix{k}(:,i) = [spike_rates(select_rep) NaN*ones(1,difc)];%[spike_rates(select_rep) NaN*ones(1,difc)];;%Katsu Corrected
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%   OUTPUT FILES   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
% foldername = 'Z:\LabTools\Matlab\TEMPO_Analysis\ProtocolSpecific\MOOG\rotation3d';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:26
    ...
    buff = sprintf(sprint_txt, FILE, fourier_ratio(i), dft_p(i), dft_p_vel(i), dft_p_acc(i), max_phase(i), shift_win(i)*.05);  
%     outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\Rotation_Frequency_26trajectories.dat'];
     outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\AIHUA_Rotation_Frequency_26trajectories.dat'];
    fid = fopen(outfile, 'a');
    fprintf(fid, '%s', buff);
    fprintf(fid, '\r\n');
    fclose(fid);
...
% outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\Rotation_Frequency_cell.dat'];%Vestibular
outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\AIHUA_Rotation_Frequency_cell.dat'];%Visu
fid = fopen(outfile, 'a');
fprintf(fid, '%s', buff);
fprintf(fid, '\r\n');
fclose(fid);
