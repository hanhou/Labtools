% Modify %%%%%%%%Rotaiont_3D.m -- For Que's RVOR/pursuit, Frontal tuning is
% picked up from Rotation 3D protocol
%--	Katsu
%-----------------------------------------------------------------------------------------------------------------------
function Rotation_Frontal(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE);
...
%                                                                              unique_elevation(3)=0 deg
    if i==1
        select = logical( (azimuth==unique_azimuth(1)) & (elevation==unique_elevation(3) ) & (condition_num==unique_condition_num(k)) );
    elseif i==2
        select = logical( (azimuth==unique_azimuth(1)) & (elevation==unique_elevation(4) ) & (condition_num==unique_condition_num(k)) );
    elseif i==3
        select = logical( (azimuth==unique_azimuth(1)) & (elevation==unique_elevation(5) ) & (condition_num==unique_condition_num(k)) );
    elseif i==4
        select = logical( (azimuth==unique_azimuth(5)) & (elevation==unique_elevation(4) ) & (condition_num==unique_condition_num(k)) );
    elseif i==5
        select = logical( (azimuth==unique_azimuth(5)) & (elevation==unique_elevation(3) ) & (condition_num==unique_condition_num(k)) );
    elseif i==6
        select = logical( (azimuth==unique_azimuth(5)) & (elevation==unique_elevation(2) ) & (condition_num==unique_condition_num(k)) );
    elseif i==7
        select = logical( (azimuth==unique_azimuth(1)) & (elevation==unique_elevation(1) ) & (condition_num==unique_condition_num(k)) );
    elseif i==8
        select = logical( (azimuth==unique_azimuth(1)) & (elevation==unique_elevation(2) ) & (condition_num==unique_condition_num(k)) );
    end
%             select = logical( (azimuth==unique_azimuth(i)) & (elevation==zero_elevation ) & (condition_num==unique_condition_num(k)) );
...
%                 
                ...
                resp_mat_ste(k, i) = resp_mat_std(k, i)/ sqrt(length(find( (azimuth==unique_azimuth(i))&(condition_num==unique_condition_num(k)) )) );
            else
...
% for Rotation 0-45-90-135--------

resp_mat_tran(:,1) = resp_mat(:,1);
resp_mat_tran(:,2) = resp_mat(:,2);
resp_mat_tran(:,3) = resp_mat(:,3);
resp_mat_tran(:,4) = resp_mat(:,4);
resp_mat_tran(:,5) = resp_mat(:,5);
resp_mat_tran(:,6) = resp_mat(:,6);
resp_mat_tran(:,7) = resp_mat(:,7);
resp_mat_tran(:,8) = resp_mat(:,8);
resp_mat_tran(:,9) = resp_mat_tran(:,1);

resp_mat_ste_tran(:,1) = resp_mat_ste(:,1);
resp_mat_ste_tran(:,2) = resp_mat_ste(:,2);
resp_mat_ste_tran(:,3) = resp_mat_ste(:,3);
resp_mat_ste_tran(:,4) = resp_mat_ste(:,4);
resp_mat_ste_tran(:,5) = resp_mat_ste(:,5);
resp_mat_ste_tran(:,6) = resp_mat_ste(:,6);
resp_mat_ste_tran(:,7) = resp_mat_ste(:,7);
...
az_plot(k)=(az(k)./45)+1;% x-axis convert to 1----9
...
for k=1: length(unique_condition_num)     
    ...
    hold on;
    plot([az_plot(k),az_plot(k)],[min(resp_mat(k,:)),max(resp_mat(k,:))],'r-');
    hold on;
...
text(13,length(unique_condition_num),'spont                  min             max                        prefer Rot.Azimuth                  p-ANOVA');

for k=1:length(unique_condition_num) 
% Now write
    text(0,length(unique_condition_num)-k,h_title{k});
    text(15,length(unique_condition_num)-k, num2str( spon_resp));
    Min_resp(k)=min(resp_mat(k,:));
    text(27,length(unique_condition_num)-k, num2str( Min_resp(k) ) );
    Max_resp(k)=max(resp_mat(k,:));
    text(37,length(unique_condition_num)-k, num2str( Max_resp(k) ) );
...
%     text(-47,-40, 'Azim: 0       45       90        135        180       225        2700        315        360');  
...
outfile = [BASE_PATH 'ProtocolSpecific\MOOG\rotation3d\Frontal_Rotaion_pursuit.dat'];
printflag = 0;
...
fid = fopen(outfile, 'a');
if (printflag)
    fprintf(fid, 'FILE\t         Spon\t Vest-min\t Visual-min\t Vest-max\t Visual-max\t VestPreDir\t VisualPreDir\t VestANOVA\t VisualANOVA\t');
fprintf(fid, '\r\n');
end
fprintf(fid, '%s', buff);
fprintf(fid, '\r\n');
