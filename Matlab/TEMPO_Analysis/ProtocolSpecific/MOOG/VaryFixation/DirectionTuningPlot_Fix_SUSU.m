...
%-- Hijacked for SUSU analysis of noise correlation, SF 12/08
%-----------------------------------------------------------------------------------------------------------------------
function DirectionTuningPlot_Fix_SUSU(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
%% data dividing
Path_Defs;
...
%%
%get the firing rates, spike times, and events for all the trials 
...
% end
% 
% channelsignal = [channelsignal(end-1) channelsignal(end)];
% channelcount=2;
% %here hard coded to take last 2 channels only
% 
...
% text(15,length(unique_stim_type)*(channelcount-1),'preferred           p                  noisecorrelation                similarity');
% count = 0;
% for k=1:length(unique_stim_type) 
%     for c=1:2
%         count = count+1;
...
%         text(53,length(unique_stim_type)*(channelcount-1)-count, num2str(corrcoef_r_unit(k)) ); 
%     end
% end
% axis off;
...
% outfile = [BASE_PATH 'ProtocolSpecific\MOOG\VaryFixation\Spike2_SUSU_noisecorr.dat'];
% printflag = 0;
...
% fid = fopen(outfile, 'a');
% if (printflag)
%     fprintf(fid, 'FILE\t    stim \t pref1 \t p1 \t pref2 \t p2 \t DiffPref \t r_nc \t r_s');
%     fprintf(fid, '\r\n');
% end
% for k=1:length(unique_stim_type)
%     preferred(1) = az(1,k);
...
%     buff = sprintf(sprint_txt, FILE, unique_stim_type(k), preferred(1), pvalue(1), preferred(2), pvalue(2), prefdiff , noise_r_stim(k), corrcoef_r_unit(k));
%     fprintf(fid, buff);
%     fprintf(fid, '\r\n');
% end
% 
...
repetition = floor( length(azimuth) / (length(unique_azimuth)*length(unique_elevation)*length(unique_stim_type)));
...
%         spike_rates_z = temp_spike_rates_z(1:end);
        ...
        for k=1:length(unique_stim_type)  
            ...
            %convert response matrix to a row vectors based on channel and stim
            %type
...
%         outfile = ['Z:\Users\Yong\noisecorr_1000msnew.dat'];
 %       buff = sprintf(sprint_txt,FILE,noise_r_stim(c,:));  
 %        buff = sprintf(sprint_txt,FILE,corrcoef_r_twounit(c),corrcoef_p_twounit(c) ); 
    else
  %      buff = sprintf(sprint_txt,FILE,noise_r_stim(c,:)); 
        buff = sprintf(sprint_txt,FILE, ff{c} );
  %      buff = sprintf(sprint_txt,FILE,corrcoef_r_twounit(c),corrcoef_p_twounit(c) ); 
    end
    outfile = ['Z:\Users\Yong\noisecorr_ff.dat'];
...
ndunique_condition_num = munique(condition_num');


from here on is commented out
% Assign titles
