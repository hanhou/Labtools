...
function DI = cross_covariance_3D(resp_mat, unique_stim_type, unique_condition_num, bin, method, showplots);
...
for n = 1:length(unique_stim_type)
    for k = 1:length(unique_condition_num)
        ...
        for j = 1:length(y)
            ...
            yi_expand{k,n}(j,:) = [yi{k,n}(j, length(xi) - (shift/bin)+1 : length(xi))' ; yi{k,n}(j,:)' ; yi{k,n}(j, 1 : shift/bin)'];
            yi_shift{k,n}(j,:) = [yi{k,n}(j,:)' ; yi{k,n}(j, 1 : 2*shift/bin)'];
        end
...
% %         end
% %     end
% %     cov_self = cov_self((shift/bin)/2 + 1 : length(xi) - (shift/bin)/2);
...
% %     DI_self(n) = (DI_index - (shift/bin)/2 - 1) * bin / unique_condition_num(3)
    ...
    cov_MZ = cov_MZ((shift/bin)/2 + 1 : length(xi) - (shift/bin)/2);
    ...
    cov_MP = cov_MP((shift/bin)/2 + 1 : length(xi) - (shift/bin)/2);
    ...
    cov_ZP = cov_ZP((shift/bin)/2 + 1 : length(xi) - (shift/bin)/2);
