...
%-----------------------------------------------------------------------------------------------------------------------
function DirectionTuningPlot_1D_Az_VF(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
%--------------------------------------------------------------------------
% Code for replacing temp_spike_rates (from .htb) with rates computed from 
% a SpikeSort2 .mat file.  Does not replace temp_spike_data.  (CRF, 9/2005)
%--------------------------------------------------------------------------
% matfile = [PATH(1:end-4) 'CED\SortedSpikes\' FILE(1:end-4) '.mat'];
% load(matfile);
% 
% for a = BegTrial:EndTrial
%     
%     % translate spike times for each trial into an array of 0's and 1's
%     trial_length = spsData.spikeInfo(a).endTime - spsData.spikeInfo(a).startTime;
%     trial_spikes = zeros(1,trial_length);
%     trial_spikes(spsData.spikeInfo(a).pSpikeTimes - spsData.spikeInfo(a).startTime) = 1;
%     
%     % count spikes in the designated interval, taking into account the higher sampling rate
%     temp_spike_rates(a) = sum(trial_spikes(25000 + (StartOffset*25) : 75000 + (StopOffset*25)));
%     
% end
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% Code for omitting trials in the middle of a session (manually enter trial #'s)
% (CRF, 9/2005)
%--------------------------------------------------------------------------
% temp_azimuth = [temp_azimuth(1:186) temp_azimuth(280:372)]
% temp_stim_type = [temp_stim_type(1:186) temp_stim_type(280:372)]
% temp_amplitude = [temp_amplitude(1:186) temp_amplitude(280:372)]
% temp_fix_x = [temp_fix_x(1:186) temp_fix_x(280:372)]
% temp_fix_y = [temp_fix_y(1:186) temp_fix_y(280:372)]
% temp_spike_rates = [temp_spike_rates(1:186) temp_spike_rates(280:372)]
% BegTrial = 1; EndTrial = 279;
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
...
repetition = floor( length(spike_rates) / (length(unique_azimuth)*length(unique_condition_num)*length(unique_stim_type)) );
vari_straight = (1+length(unique_condition_num))/2;
% response matrix
...
%    for k=1:length(unique_condition_num)
    for k=1:length(unique_condition_num)        
        for n=1:length(unique_stim_type)
         ...
         %       select = logical( (azimuth==unique_azimuth_8(i)) & (condition_num==unique_condition_num(vari_straight)) & (stim_type==unique_stim_type(n)) );
        %        select = logical( (azimuth==unique_azimuth(i)) & (condition_num==unique_condition_num(vari_straight)) & (stim_type==unique_stim_type(n)) );
                if (sum(select) > 0) 
                    count = count+1;
...
%     buff = sprintf(sprint_txt, FILE,resp_mat_1, resp_mat_std_1.^2 );
%     if s==1
...
eye_x_left_temp(:,:) = eye_x_left_temp_temp(:, fix_x==0);
eye_y_left_temp(:,:) = eye_y_left_temp_temp(:, fix_x==0);
eye_x_right_temp(:,:) = eye_x_right_temp_temp(:, fix_x==0);
eye_y_right_temp(:,:) = eye_y_right_temp_temp(:, fix_x==0);
...
fclose(fid);  
%end
% % write to different files if there are different stimuli conditions
% if length(unique_stim_type)==3
%     buff = sprintf(sprint_txt, FILE, length(unique_stim_type), rr_stim(1,:) );
%     outfile = ['Z:\Users\Yong\variancetomeancorrelation50ms_ves.dat'];
%     printflag = 0;
%     if (exist(outfile, 'file') == 0)   % file does not yet exist
%         printflag = 1;
%     end
...
%     fclose(fid);  
%     
%     buff = sprintf(sprint_txt, FILE, length(unique_stim_type), rr_stim(2,:) );
%     outfile = ['Z:\Users\Yong\variancetomeancorrelation50ms_vis.dat'];
%     printflag = 0;
%     if (exist(outfile, 'file') == 0)   % file does not yet exist
%         printflag = 1;
%     end
...
%     fclose(fid);
%     
%     buff = sprintf(sprint_txt, FILE, length(unique_stim_type), rr_stim(3,:) );
%     outfile = ['Z:\Users\Yong\variancetomeancorrelation50ms_comb.dat'];
%     printflag = 0;
%     if (exist(outfile, 'file') == 0)   % file does not yet exist
%         printflag = 1;
%     end
