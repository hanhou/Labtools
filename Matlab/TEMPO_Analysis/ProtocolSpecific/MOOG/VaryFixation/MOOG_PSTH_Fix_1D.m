...
%             if (sum(select) > 0)
%                 resp_mat(i,n,k) = mean(spike_rates(select));
%             else
...
% for n = 1:length(unique_stim_type)
%     clear x;
%     x = find(resp_mat(:,n,2) == max(max(resp_mat(:,n,2))));
%     ii{n} = x(1);
% end
% 
% 
...
% 
% for n=1:length(unique_stim_type)
%     select = logical( (azimuth==unique_azimuth(ii{n})) & (condition_num==unique_condition_num(2)) & (stim_type==unique_stim_type(n)) ); 
%     resp{n} = mean(spike_rates(select));
...
%         time_step=1;
%     end
%     count_y_trial{n}(:,:) = temp_count;  % each trial's PSTH 
...
%            FILE, rate_y{n}, stim_text);
% 	outfile = ['C:\MATLAB6p5\work\new_PSTH_fix\PSTH_fix_1D.dat'];
% 	printflag = 0;
...
out_values_vis = gaussfit_new_1D(dat2, row_m{2}, col_m{2}, FILE, DC_norm(2,:), fourier_ratio(2,:), reject{2}, plot_col, plot_row, no_fit(2), 'Visual');
figure(11); title([FILE '  Vestibular']); orient landscape; set(11,'Position', [25,50 1250,750]);
out_values_ves = gaussfit_new_1D(dat1, row_m{1}, col_m{1}, FILE, DC_norm(1,:), fourier_ratio(1,:), reject{1}, plot_col, plot_row, no_fit(1), 'Vestibular');
figure(13); title([FILE '  Combined']); orient landscape; set(13,'Position', [25,50 1250,750]);
out_values_comb = gaussfit_new_1D(dat3, row_m{3}, col_m{3}, FILE, DC_norm(3,:), fourier_ratio(3,:), reject{3}, plot_col, plot_row, no_fit(3), 'Combined');
