...
function HeadingDis_cum_shiftwindow(data, Protocol, Analysis, SpikeChan, StartEventBin, StopEventBin,StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, batch_flag);
...
set(figure(777),'color','w');
if ~isempty(batch_flag); set(777,'visible','off'); end


clf;
[ax,h1,h2] = plotyy(xx,Thresh_neu_shift(1,:),xx,CP_all_shift(1,:));
...
lims = axis; hold on; 
plot([1000 1000],[1 lims(4)],'k');
ylim([1 200]);

axes(ax(2)); hold on; 
set(plot(xx(p_shift(1,:)<0.05),CP_all_shift(1,p_shift(1,:)<0.05),'o'),'markerfacecolor',get(h2,'color'),'markersize',10);   % Sig. CP
plot([lims(1) lims(2)],[0.5 0.5],'color',get(h2,'color'),'linestyle','--');
ylim([0.3 1]); 
set(gca,'ytick',0.3:0.1:1);

set(findall(gcf,'FontSize',10),'FontSize',15);
xlabel(['Center of ' num2str(window_interval) ' ms time window (ms)']);
...
if ~isempty(batch_flag)
    ...
    buff = sprintf(sprint_txt, [FILE '_' num2str(SpikeChan)], endloop, Thresh_neu_shift(1,:), CP_all_shift(1,:), p_shift(1,:));
