...
%-----------------------------------------------------------------------------------------------------------------------
%% HH20141026
% For yuchen's dt-experiments. HH201604

function Psychometric_HH_dt_yuchen(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, batch_flag);
...
for cc = unique_conflict_time(unique_conflict_time~=0)'
    ...
    stim_type (stim_type == 3 & conflict_time_per_trial == cc) = temp_stim_type;
    stim_type_names = [stim_type_names,['Comb_' num2str(cc)]];
end

stim_type_colors = [0 0 1; 1 0 0; 0 0.8 0.4; 1 0.8 0; 0 1 1];
unique_stim_type = munique(stim_type');  % Update unique_stim_type
...
for c = 1:length(unique_motion_coherence) % different coherence level
    for condition = 1:length(unique_stim_type)
...
symbo{1,1} = 'bo';    symbo{1,2} = 'ro';    symbo{1,3} = 'go';  symbo{1,4} = 'yo'; symbo{1,5} = 'co';
symbo{2,1} = 'b^';    symbo{2,2} = 'r^';    symbo{2,3} = 'g^';
symbo{3,1} = 'bv';    symbo{3,2} = 'rv';    symbo{3,3} = 'gv';

fitline{1,1} = 'b-';    fitline{1,2} = 'r-';    fitline{1,3} = 'g-';  fitline{1,4} = 'y-'; fitline{1,5} = 'c-';
