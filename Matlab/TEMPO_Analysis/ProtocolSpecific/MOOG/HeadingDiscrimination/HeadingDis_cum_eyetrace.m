...
if ~isnan(data.one_time_params(LEFT_EYE_X_CHANNEL))
    LEYE_H = data.one_time_params(LEFT_EYE_X_CHANNEL);
end

if ~isnan(data.one_time_params(LEFT_EYE_Y_CHANNEL))
    LEYE_V = data.one_time_params(LEFT_EYE_Y_CHANNEL);
end

if ~isnan(data.one_time_params(RIGHT_EYE_X_CHANNEL))
    REYE_H = data.one_time_params(RIGHT_EYE_X_CHANNEL);
end

if ~isnan(data.one_time_params(RIGHT_EYE_Y_CHANNEL))
...
for j = 1:size(align_markers,1)    % For each desired marker
    ...
    for i = 1:sum(select_trials)
        ...
        if j == 1
            eye_offsetX(i) = mean(eyeX_aligned{1,1}(i,winBeg_eye : winBeg_eye + 500/eye_timeWin));
            eye_offsetY(i) = mean(eyeY_aligned{1,1}(i,winBeg_eye : winBeg_eye + 500/eye_timeWin));
        end
...
j = 2; % Saccade onset
plot_time_win = [-1000 200];

eyeX = eyeX_aligned{1,j};
eyeY = eyeY_aligned{1,j};

plot_win = plot_time_win(1)<= eyeX_aligned{2,j} & eyeX_aligned{2,j} <= plot_time_win(2);

set(figure(101),'pos',[65 290 1105 649]); clf
subplot(2,3,[1 2 4 5]);

plot(reshape(eyeX(:,plot_win),1,[]),reshape(eyeY(:,plot_win),1,[]),'b.');

max_axis = max(abs([xlim ylim]));
axis([-max_axis max_axis -max_axis max_axis]);
axis square;

subplot(2,3,3);
plot(eyeX_aligned{2,j},eyeX','b'); title('LEYE\_X');
subplot(2,3,6);
plot(eyeY_aligned{2,j},eyeY','b'); title('LEYE\_Y');
