...
% Changed for yuchen's dt-experiments HH201604
%-----------------------------------------------------------------------------------------------------------------------

function HeadingDis_cum_PSTH_HH_dt_yuchen_patch_for_HD(data, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol, batch_flag);
...
conflict_time_per_trial = data.moog_params(CONFLICT_TIME,select_trials,MOOG);

unique_heading = munique(heading_per_trial');
unique_conflict_time = munique(conflict_time_per_trial');

%%%%%%%%%%% Turn conflict_time to stim_type %%%%%%%%%%%%%%
temp_stim_type = 3;
stim_type_names = {'All','Vest','Vis','Comb_0'};

for cc = unique_conflict_time(unique_conflict_time~=0)'
    temp_stim_type = temp_stim_type + 1;
    stim_type_per_trial (stim_type_per_trial == 3 & conflict_time_per_trial == cc) = temp_stim_type;
    stim_type_names = [stim_type_names,['Comb_' num2str(cc)]];
end

stim_type_colors = [0 0 1; 1 0 0; 0 0.8 0.4; 1 0.8 0; 0 1 1];
unique_stim_type = munique(stim_type_per_trial');  % Update unique_stim_type
repetitionN = floor(sum(select_trials) / length(unique_heading) / length(unique_stim_type)) ;

% ------------ Patch in HD_dt cells. HH20160919 -----------
unique_stim_type = 1:3; % Fix stim_type to 1:3 for patching.

% for i = 1:length(select_trials)
%     if stim_type_per_trial(i) == 3 && conflict_time_per_trial(i)== -250
%         stim_type_per_trial(i)= 4
%     elseif  stim_type_per_trial(i) == 3 && conflict_time_per_trial(i)== 250
%         stim_type_per_trial(i)= 5
%     end
% end
%
% stim_type_names = {'All','Vest','Vis','Comb_0','Comb_-250','Comb_250'}; % stim_type = 0, 1, 2, 3, 4, 5
% stim_type_colors = [0 0 1; 1 0 0; 0 0.8 0.4; 1 0.8 0; 0 1 1];
...
%
% Note that this influence is global!!
HD_dt_move_forward = 125; % ms   (1750-1500)/2 = 125
...
for j = 1:size(align_markers,1)    % For each desired marker
    ...
    if j == 1  % Only affect the stim-on alignment
        align_offsets(:,j) = align_offsets(:,j) + ceil(HD_dt_move_forward / spike_timeWin);
    end
    % ------------ End patch in HD_dt cells. HH20160919 -----------
    
    spike_aligned{1,j} = zeros(sum(select_trials), ceil((align_markers{j,3} - align_markers{j,2}) / spike_timeWin) + 1); % Preallocation
    ...
    for jj = 1:length(align_markers{j,5})
        ...
        % ------------ Patch in HD_dt cells. HH20160919 -----------        %
        % VSTIM_OFF_CD  means --> 1500 ms Gaussian ends (125 ms before the real VSTIM_OFF_CD)
        if j == 1 && align_markers{j,5}(jj) == VSTIM_OFF_CD % Only affect the stim-off marker in stim-on alignment
            align_offsets_others{j}(:,jj) = align_offsets_others{j}(:,jj) - ceil(HD_dt_move_forward / spike_timeWin);
        end
        % ------------ End patch in HD_dt cells. HH20160919 -----------
...
if if_figure
    ...
    for stim_type = 1:length(unique_stim_type)
        ...
        if length(find_trial_pref) >= num_trials_for_raster && length(find_trial_null) >= num_trials_for_raster
            ...
            select_for_raster_pref = sort_pref(round(linspace(round(length(sort_pref)*0.05),round(length(sort_pref)*0.95),num_trials_for_raster))); % randomly choose 20 trials to plot.
            ...
            plot([0 0],ylim,'k-'); plot([align_offsets_others{1}(1) align_offsets_others{1}(1)],ylim,'k-'); plot(xlim,0.5 + [num_trials_for_raster num_trials_for_raster],'k--');
        end
    end
    % SetFigure(15);
    drawnow;
...
sort_info = {
    ...
    % In each subplot (max nested level: 2)
    {    %  Variable(s) ,  Logical,  Values  , Colors,  LineStyles,    Notes,   Errorbar? (0:Nothing; 1:95% CI; 2: p<0.05; 3:both)
    'abs(heading_per_trial)''',{'<','>='}, [abs(unique_heading(round(fix(length(unique_heading)/2)/2))) abs(unique_heading(round(fix(length(unique_heading)/2)/2)))], ...
    [0 0 0; 1 0 0], {}, {['|Heading| < ' num2str(abs(unique_heading(round(fix(length(unique_heading)/2)/2))))], ...
    ['|Heading| >= ' num2str(abs(unique_heading(round(fix(length(unique_heading)/2)/2))))]} , 999;
    'choice_per_trial', {'=='}, [PREF NULL], [], {'-'; '--'}, choose_names, 2;
    }
    }
        ...
        if ~isempty(k)   % We have this condition
            ...
            for tt = 1:length(rate_ts{j})
                ...
                [auROC, ~, perm] = rocN(PSTH{j,ALL_CHOICE,1}.raw{2*k-1}(:,tt),...
                    PSTH{j,ALL_CHOICE,1}.raw{2*k}(:,tt), [], CP_perm_N);      % Default: first larger
...
ChoicePreference = nan(length(choice_or_mod_pref_timewin),length(unique_stim_type));
ChoicePreference_pvalue = nan(length(choice_or_mod_pref_timewin),length(unique_stim_type));
...
if if_figure
    ...
    for k = 1:length(unique_stim_type)
        ...
        text(max(unique_heading)*0.5,0.5-k*0.07,sprintf('%6.3g',Psy_para(2)),'color',stim_type_colors(unique_stim_type(k),:));
    end
...
if exist('CP','var')  % Full version
    result = PackResult(FILE, SpikeChan, repetitionN, unique_stim_type, HD_dt_patched, ... % Obligatory!!
