...
sigma=15; dist=(-(sigma*5):(sigma*5))'; gaus=1/(sigma*sqrt(2*pi))*exp(-((dist.^2)/(2*sigma^2)))*1000; %sigma in ms, 5 X sigma. Gaussian for smoothing the PSTH (to form the SDF)
    ...
    for cell=1:cells
        ...
        if max(resp_mat{1,k,cell})<1, eval(sprintf('%s.sdf%s{cell}(1:%u,:)=NaN;',var,stim{unique_stim_type(k)},length(unique_heading))); %the cell did not fire
            ...
            for i = 1:length(unique_heading)
                ...
                eval(sprintf('%s.sdf%s{cell}(i,:)=decimate(mean(temp_sdf,2),10);',var,stim{unique_stim_type(k)}));
            end
        end
    end
end
