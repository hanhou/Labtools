function result = CP_HH_GPU(headings,choices,spike_counts,CP_p_method, IF_FIGURE)
...
if IF_FIGURE
    ...
    % Fitting
    [Psy_bias,Psy_thres] = cum_gaussfit_max1([unique_heading, rightward_prop, num_headings]);
...
if ~if_fake
    ...
    [Neu_bias_anti,Neu_thres_anti] = cum_gaussfit_max1([unique_heading(unique_heading~=0), rightward_prop_neuro_anti]);
...
%         tic
%         parfor i = 1 : 1000   % For each permutation (total times: CP_p_method)
%             % Permuted CP_0
...
%         toc
        ...
        tic
        rates = gpuArray(spike_counts_0_all_choices);
        n_pref_gpu = length(spike_counts_allheadings_grouped{h_0,result.pref}) * gpuArray.ones(1,100);
        CP_0_perm = arrayfun(@PermutedCP, n_pref_gpu);%,'UniformOutput',0);
        toc
...
%  Nested function
    function CP_perm = PermutedCP(n_pref)
        %         rates = gpuArray(spike_counts_0_all_choices);
        %         randperm(spike_counts_0_all_choices);
        
        %         rand_choice_pref = gpuArray(randperm(length(rates),n_pref));
        %         rand_choice_null = gpuArray(setdiff(1:length(rates),rand_choice_pref));
        
%         rand_choice_pref = gpuArray.rand(100,1);
%         rand_choice_null = gpuArray.rand(100,1);
%         CP_perm = rocN_GPU((rand_choice_pref),(rand_choice_null));
        CP_perm = 3+4;
    end
