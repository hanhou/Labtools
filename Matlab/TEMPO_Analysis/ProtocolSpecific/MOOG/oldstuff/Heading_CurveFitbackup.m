    ...
    for j=1:num_fits
        ...
        if ~use_backdoor_fit  % there is no backdoor, you are not reading this.
            ...
            for n=1:minimization_iterations
                sprintf('iter %d %s stim %d %s', n, func2str(fitting_models(j)), ...
                    fitting_stims(i), FILE)
            ...
            if build_backdoor_fit
                ...
                fprintf( fid, '\n%s %s %s %s %d %d %.16e ', FILE, func2str(fitting_models(j)), ...
...
for i=1:num_stims
    ...
    % sum variance over all gazes, repitions, and points to get total ss_yy
    ...
    for i=1:num_stims
        ...
        for j=1:plot_contour_compares_step:plot_contour_num_compares
            ...
            for gas=1:max_3_gazes
                ...
                [C h] = contourf(plot_azimuth,plot_elevation,plot_primary_data);
                %colorbar;
                ...
                %subplot('Position',[0 0.5 0.5 0.5]);
                [C h] = contourf(plot_azimuth,plot_elevation,plot_secondary_data);
                %colorbar;
                set(gca,'ydir','reverse','xdir','reverse','xtick',x_tick,'ytick',y_tick);
                axis([x_min,x_max,y_min,y_max]); caxis('auto'); if plot_clabels, clabel(C,h), end;
                %xlabel('azimuth'); ylabel('elevation'); 
      ...
      %         end
            ...
            %plot 7 params
            str_mapping=cell(num_params(i,j),1);
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_AZIMUTH)) = cellstr('Azimuth');
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_ELEVATION)) = cellstr('Elevation');
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_NONLIN)) = cellstr('Non-Linearity');
            %str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_NONLIN180)) = cellstr('Non-Linearity 180');
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_AMPLITUDE)) = cellstr('Amplitude');
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_DC_OFFSET)) = cellstr('DC Offset');
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180)) = cellstr('Weighting 180');
            str_mapping(curvefit_parameter_mapping(CURVEFIT_PARAM_NONLIN2)) = cellstr('Nonlin2');
            for k=1:num_params(i,j)
                str= sprintf('%s  = %f', char(str_mapping(k)), best_x(i,j,k));
                h = text(0,-.04-.02*k,str);
               set( h,'Interpreter', 'none'); set(h,'FontName','FixedWidth','FontSize',10);
           end
            
            %plot chi stats
            for c=1:num_fits
            str=sprintf('%s \nChi2 = %f \nChi2P = %f', func2str(fitting_models(c)),chi2_stat(c), chi2p_stat(c));
