...
%-----------------------------------------------------------------------------------------------------------------------
function F = Curvefit_cos_tuning_8p_asp(xp,azimuth,elevation)
...
if (nonlin2 ~= 0)
    ga=( exp(nonlin2*cos(rott)) - 1 ) / (nonlin2);
else
    ga = cos(rott);
end

if (nonlin2~=0)
    ha=( exp(nonlin2*cos(rott-pi)) - 1 ) / (nonlin2);
else
    ha = cos(rott-pi);
end

if (nonlin ~= 0)
    gb = ( exp(nonlin*cos(rotp)) - 1 ) / (nonlin);
else
    gb = cos(rotp);
end
if (nonlin180~=0)
    hb = ( exp(nonlin180*cos(rotp-pi)) - 1 ) / (nonlin180);
else
    hb = cos(rotp-pi);
end
g=ga.*gb;
