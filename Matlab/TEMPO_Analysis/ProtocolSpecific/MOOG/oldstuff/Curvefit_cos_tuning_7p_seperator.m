%-----------------------------------------------------------------------------------------------------------------------
%-- Curvefit_cos_tuning_7p.m -- This is the 7 parameter model of cos tuning as 
%--   a function of the position on the sphere (azimuth, elevation).
%-- Created - pwatkins, 4/04
%-----------------------------------------------------------------------------------------------------------------------
function [main,secondary] = Curvefit_cos_tuning_7p_seperator(xp,azimuth,elevation)
...
nonlin180 = xp(curvefit_parameter_mapping(CURVEFIT_PARAM_NONLIN180));
...
if ~isreal(F) | ~all(isfinite(F))
    error(sprintf( 'Curvefit_cos_tuning_7p returning nonreal or nonfinite value' ));
end
