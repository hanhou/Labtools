...
psth_windows = [-500 2000; -1000 1500; -1800 500]; % Pre-trigger and Post-trigger PSTH windows (cover all possible range)
...
for j = 1:length(align_markers)    % For each desired marker
    ...
    % Align spikes    
    spike_aligned{j} = zeros(sum(select_trials),ceil((psth_windows(j,2) - psth_windows(j,1)) / spike_timeWin) + 1);
...
if memSacOK
    ...
    temporal_Slice = {
        ...
        75 275 VSTIM_ON_CD 'LS' 'g';               % LS: 75 ~ 275 ms (04);
        -600 -50 VSTIM_OFF_CD 'Sustained'  'r';          % Sustained: 200 ~ 800 ms (04);
        -200 -25 SACCADE_BEGIN_CD 'pre-S' 'b';         %  pre-S: -200 ~ 0 (07);
        -50 50 SACCADE_BEGIN_CD 'S-Co' 'c';  % Saccade-coincident: -25 ~ 75 (07)
        50 400 SACCADE_BEGIN_CD 'post-S' 'm';     % post-S: 50 ~ 500 (07)
        };
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
