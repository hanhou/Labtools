...
spike_data = reshape(spike_data_good, data_duration, length(trials)); %length(trials)
...
%     end
    ...
    target_onset_bin(i) = floor(stim_start(i)/timebin) + 1;
%     target_onset_bin(i) = floor(stim_on_pulse(i)/timebin) + 1;
...
%             targ_spike_raster{i,k}(1:targ_raster_start,l) = spike_rate_raster{i,k}((stim_on_pulse(act_found(l))-targ_raster_start)+1:stim_on_pulse(act_found(l)),l);
%             targ_spike_raster{i,k}(targ_raster_start+1:targ_raster_start+data_duration-targ_raster_end,l) = spike_rate_raster{i,k}(stim_on_pulse(act_found(l))+1:stim_on_pulse(act_found(l))+data_duration-targ_raster_end,l);
...
for i = 1:length(unique_heading)
    ...
    for j  = 1:length(unique_amplitude)
        ...
        for l = 1:length(act_found)
            %Target aligned
            subplot(length(unique_amplitude),2,2*j-1), plot(-200:800,spike_raster{i,j}((stim_start(act_found(l))-200:(stim_start(act_found(l)))+800),l),'k.');
            hold on
            %             subplot(4,2,2*j-1), plot(-100:5:200, .001*diode_adj{i,j}((targ_on_diode(act_found(l))/5-100/5:(targ_on_diode(act_found(l)))/5+200/5),l));
            subplot(length(unique_amplitude),2,2*j-1), plot(-200:timebin:800, targ_average_spikes{i,j}(targ_bin_start-200/timebin:targ_bin_start+800/timebin), 0,1:.5:40,'k');
            set(gca,'ylim',[0 150]);
            xlabel('Target onset');
            ylabel('Spikes/s');
