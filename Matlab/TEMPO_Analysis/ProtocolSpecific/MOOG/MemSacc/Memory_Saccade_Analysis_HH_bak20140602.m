...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
align_markers = [4 5 7];  % Desired markers: target onset & target offset & sac onset
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
temporal_Slice = { -500 -200 04 'Background' 'k';          % Background: -500 ~ -100 ms (04)
                   75 400 04 'LS' 'm';               % LS: 75 ~ 275 ms (04);
                  25 500 05 'Memory'  'g';          % Sustained: 200 ~ 800 ms (04);
                   -300 -25 07 'pre-S' 'b';         %  pre-S: -200 ~ 0 (07);
                   -25 75 07 'S-Co' 'c';  % Saccade-coincident: -25 ~ 75 (07)
                   100 500 07 'post-S' 'r';     % post-S: 50 ~ 500 (07)
                   };   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = 1:length(align_markers)
    ...
    subplot(3,3,1);
    if align_markers(j) == 4 % Targ On
        plotXLim = [-500 1500];
        title([FILE '  Target On (04)']);
    elseif align_markers(j) == 7 % Sac On
        plotXLim = [-1500 500];
        title([FILE '   Saccade On (07)']);
    elseif align_markers(j) == 5 % Targ Off
        plotXLim = [-1000 1000];
        title([FILE '   Target Off (05)']);
    end
...
%         h_p = polar([vectSum{sliceN} vectSum{sliceN}]/180*pi,[0 vectAmp{sliceN}],[temporal_Slice{sliceN,5} '-']);
        h_p = polar([vectSum{sliceN} vectSum{sliceN}]/180*pi,[max(cell2mat(resp))*1.1 max(cell2mat(resp))*1.3],[temporal_Slice{sliceN,5} '-']);
        set(h_p,'LineWidth',5);
    end
%     
