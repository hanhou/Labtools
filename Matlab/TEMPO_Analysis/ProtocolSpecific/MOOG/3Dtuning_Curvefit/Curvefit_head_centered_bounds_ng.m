...
terms = CURVEFIT_NUM_FIXED_PARAMETERS_HC_NG + dyn_terms*num_gazes;
...
for j=1:num_gazes
    i = (j-1)*dyn_terms + CURVEFIT_NUM_FIXED_PARAMETERS_HC_NG+1;
    lb(i:i+dyn_terms-1) = [0     0     0];
...
% the fixed parameters are azimuth and elevation rotation angles.
x0 = rand(1,CURVEFIT_NUM_FIXED_PARAMETERS_HC_NG).*[2*pi pi 2 2] - [pi pi/2 0 0];

% create initial guess for the dynamic parameters for this model.
for j=1:num_gazes
    i = (j-1)*dyn_terms + CURVEFIT_NUM_FIXED_PARAMETERS_HC_NG+1;
    x0(i:i+dyn_terms-1) = rand(1,dyn_terms).*[0 0 1] - ...
        [-amplitude(j) -amplitude(j) 0];
end
