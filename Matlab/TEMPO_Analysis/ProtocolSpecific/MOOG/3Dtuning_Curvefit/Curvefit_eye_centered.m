...
if mod(num_params-CURVEFIT_NUM_FIXED_PARAMETERS_EC,num_gazes) ~= 0
    error('bad number of parameters in Curvefit_eye_centered');
else
    num_dyn_params = (num_params-CURVEFIT_NUM_FIXED_PARAMETERS_EC) / num_gazes;
end

P = zeros(1,CURVEFIT_NUM_FIXED_PARAMETERS_EC+num_dyn_params);  % the current parameter set
...
for f=1:num_gazes
    ...
    P(1:CURVEFIT_NUM_FIXED_PARAMETERS_EC) = xp(1:CURVEFIT_NUM_FIXED_PARAMETERS_EC);
    P(CURVEFIT_NUM_FIXED_PARAMETERS_EC+1:num_dyn_params+CURVEFIT_NUM_FIXED_PARAMETERS_EC) = ...
        xp(num_dyn_params*(f-1)+CURVEFIT_NUM_FIXED_PARAMETERS_EC+1: ...
           num_dyn_params*f+CURVEFIT_NUM_FIXED_PARAMETERS_EC); 
