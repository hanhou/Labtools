%-----------------------------------------------------------------------------------------------------------------------
%-- DirectionTuningPlot_Curvefit.m -- Fits response to model data as a function of azimuth and elevation for MOOG 3D tuning expt
%--	GCD, 6/27/03
%-- Modified for Vary_Fixation protocol   CRF + Yong, 12/03
%-- Modified for curve fitting - pwatkins, 4/04
%-----------------------------------------------------------------------------------------------------------------------
function DirectionTuningPlot_Curvefit(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, ...
...
if nargin < 13
    ...
    OPT_FILE = fullfile( OPT_FILE, 'Curvefit_default_options.m' );
end
...
% make sure we have at least one repitition.
if num_complete_repititions < 1
    sprintf('ERROR load: %s has zero complete repititions, abort.', FILE)
    return;
end
...
for i=1:num_stims
    ...
    trial_data(1:num_gazes, 1:num_unique_points, 1:num_complete_repititions) = ...
    ...
    for i=1:num_stims
        ...
        for j=1:plot_contour_compares_step:plot_contour_num_compares
            ...
            for gas=1:3
                ...
                h = text(0,0,pstr{gas}); 
                set( h,'Interpreter', 'none'); set(h,'FontName','FixedWidth','FontSize',10); axis off;
            end
...
if plot_param_export
    ...
    curvefit_repititions_export = num_complete_repititions;
end
