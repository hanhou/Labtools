...
if mod(num_params-CURVEFIT_NUM_FIXED_PARAMETERS_HC,num_gazes) ~= 0
    error('bad number of parameters in Curvefit_head_centered');
else
    num_dyn_params = (num_params-CURVEFIT_NUM_FIXED_PARAMETERS_HC) / num_gazes;
end

P = zeros(1,CURVEFIT_NUM_FIXED_PARAMETERS_HC+num_dyn_params);  % the current parameter set
...
for f=1:num_gazes
    ...
    P(1:CURVEFIT_NUM_FIXED_PARAMETERS_HC) = xp(1:CURVEFIT_NUM_FIXED_PARAMETERS_HC);
    P(CURVEFIT_NUM_FIXED_PARAMETERS_HC+1:num_dyn_params+CURVEFIT_NUM_FIXED_PARAMETERS_HC) = ...
        xp(num_dyn_params*(f-1)+CURVEFIT_NUM_FIXED_PARAMETERS_HC+1: ...
           num_dyn_params*f+CURVEFIT_NUM_FIXED_PARAMETERS_HC); 
