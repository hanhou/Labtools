...
terms = CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFADC + dyn_terms*num_gazes;

% create bounds for the fixed parameters for this model.
% the fixed parameters are azimuth and elevation rotation angles.
lb = [-2*pi  -2*pi   0   0  -50  0    0  ];
ub = [ 2*pi   2*pi  50  50   50  5e3  5e3];

% create bounds for the dynamic parameters for this model.
for j=1:num_gazes
    i = (j-1)*dyn_terms + CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFADC+1;
...
x0 = rand(1,CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFADC).*[2*pi pi 2 2 100 0 0] ...
    - [pi pi/2 0 0 50 -amplitude(2) -amplitude(2)];
