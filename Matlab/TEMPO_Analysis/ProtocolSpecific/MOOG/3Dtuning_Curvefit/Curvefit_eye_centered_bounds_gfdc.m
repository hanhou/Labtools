...
dyn_terms = 1;  % no define - dependent on this module anyways,
                % and computed elsewhere as: 
                % (total_terms - FIXED_PARAMS) / num_gazes
terms = CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFDC + dyn_terms*num_gazes;

% create bounds for the fixed parameters for this model.
% the fixed parameters are azimuth and elevation rotation angles.
lb = [-2*pi  -2*pi   0   0  -50  0    -50  0  ];
ub = [ 2*pi   2*pi  50  50   50  5e3   50  5e3];

% create bounds for the dynamic parameters for this model.
for j=1:num_gazes
    i = (j-1)*dyn_terms + CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFDC+1;
    lb(i:i+dyn_terms-1) = [0];
    ub(i:i+dyn_terms-1) = [1];
end
...
x0 = rand(1,CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFDC).*[2*pi pi 2 2 100 0 100 0] ...
    - [pi pi/2 0 0 50 -amplitude(2) 50 -amplitude(2)];

% create initial guess for the dynamic parameters for this model.
for j=1:num_gazes
    i = (j-1)*dyn_terms + CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFDC+1;
    x0(i:i+dyn_terms-1) = rand(1,dyn_terms).*[1] - [0];
end
