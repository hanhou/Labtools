...
curvefit_parameter_mapping(CURVEFIT_PARAM_DC_SLOPE) = 7;
curvefit_parameter_mapping(CURVEFIT_PARAM_DC_OFFSETB) = 8;
curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180) = 9;
curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180_SLOPE) = 9;
curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180_OFFSET) = 10;
...
if mod(num_params-CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB,num_gazes) ~= 0
    error('bad number of parameters in Curvefit_eye_centered');
else
    num_dyn_params = (num_params-CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB) / num_gazes;
end

P = zeros(1,CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB+num_dyn_params);  % the current parameter set
...
for f=1:num_gazes
    ...
    P(1:CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB) = xp(1:CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB);
    P(CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB+1:num_dyn_params+CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB) = ...
        xp(num_dyn_params*(f-1)+CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB+1: ...
           num_dyn_params*f+CURVEFIT_NUM_FIXED_PARAMETERS_EC_GFB); 
    ...
    % calculate dc offset as function of gaze angle also
    P(curvefit_parameter_mapping(CURVEFIT_PARAM_DC_OFFSET)) = ...
        P(curvefit_parameter_mapping(CURVEFIT_PARAM_DC_SLOPE))*gaze_angle(f) + ...
        P(curvefit_parameter_mapping(CURVEFIT_PARAM_DC_OFFSETB));

    % calculate weightage as function of gaze angle also
    P(curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180)) = ...
        P(curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180_SLOPE))*gaze_angle(f) + ...
        P(curvefit_parameter_mapping(CURVEFIT_PARAM_WEIGHT180_OFFSET));
