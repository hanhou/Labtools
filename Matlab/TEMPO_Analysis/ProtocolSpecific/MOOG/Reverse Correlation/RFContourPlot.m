...
ProtocolDefs;
Path_Defs;
%% Commented Figs. 4,5,6,7 by Katsu 021607, 
%% lines 106,142,149,151-154,177,183-191,222,223,252,254-258, 280(in case
%% low frequency)
%*************************************************************************%
...
FieldLoc_x=unique(data.moog_params(FIELDLOC_X,:,1));%field location: x (center for horizontal)
FieldLoc_y=unique(data.moog_params(FIELDLOC_Y,:,1));%field location: y (center for vertical)
...
for (trial=1:size(data.revcorr_params,2))
    ...
    for (delay = corrDelay)
        ...
        if (isempty(spikeTimes)) %can't do a histc on an empty matrix
            spikeHist{spikeHistIndex} (trial,:)= zeros(1, length(histEdges)-1);
        else
        ...
        if (isempty(spikeTimes_f)) %can't do a histc on an empty matrix
            spikeHist_f{spikeHistIndex}(trial,:) = zeros(1, length(histEdges)-1);
        else
...
%         text(max(x2),max(y2),' \leftarrow Fitted Line','FontSize',8)
        ...
        buff=sprintf(sprint_txt, FILE, Xpatch, Ypatch,x0(1), x0(2)*180/pi, x0(3)*180/pi, x0(4), x(1),x(2)*180/pi, x(3)*180/pi, x(4));
        outfile =['Z:\Users\Aihua\Temporal_OutputData\CurveFit_WrappedGaussian.dat'];        
        printflag = 0;
        ...
        fid = fopen(outfile, 'a');
        if (printflag)
            fprintf(fid, 'FILE\t         Xpatch\t Ypatch\t A0\t mu0\t sigma0\t DC0\t A\t mu\t sigma\t DC\t');
            fprintf(fid, '\r\n');
        end
        fprintf(fid, '%s', buff);
        fprintf(fid, '\r\n');
...
x_bin=0:bin:1;
perm_num=1000;
% figure(6);clf;orient landscape;figure(7);clf;orient landscape;
% figure(8);clf;orient landscape;
for (Xpatch = 1:numXpatches)
    for (Ypatch = 1:numYpatches) 
...
%         text(0.5*x_lim(2),0.70*y_lim(2),['rp=',num2str(rp(Xpatch,Ypatch))],'FontSize', 8);  
        ...
        %Also, write out some summary data to a cumulative summary file
        sprint_txt=['%s\t %f\t %f\t %f\t %f\t %f\t'];
        buff=sprintf(sprint_txt, FILE, Xpatch, Ypatch,Azi_f(Xpatch,Ypatch,DelayIndex),Amp_f(Xpatch,Ypatch,DelayIndex),VectSp(Xpatch,Ypatch));
        outfile =['Z:\Users\Aihua\Temporal_OutputData\CurveFit_VectorSumPValue.dat'];        
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
        fid = fopen(outfile, 'a');
        if (printflag)
            fprintf(fid, 'FILE\t         Xpatch\t Ypatch\t PrefAzi\t VectorAmp\t p_VectorSum\t');
            fprintf(fid, '\r\n');
        end
        fprintf(fid, '%s', buff);
        fprintf(fid, '\r\n');
...
figure(3);clf;orient landscape;%figure(8);clf;
subplot(2,2,1);contourf(x_interp,y_interp, flipud(z_gauss1));colorbar;grid;axis([-1*View_Dist View_Dist -1*View_Dist View_Dist]);
...
figure(3);subplot(2,2,2);contourf(x_interp,y_interp,flipud(z_gauss2));colorbar;grid;axis([-1*View_Dist View_Dist -1*View_Dist View_Dist]);
clear x;x=[-1 -0.4142 0 0.4142 1]*View_Dist;
set(gca,'XTick',x);
ticks = atan(x/View_Dist)/pi*180;
set(gca,'XTickLabel', ticks);
...
figure(3);subplot(2,2,3);contourf(x_interp_new,y_interp_new,zz);colorbar;grid;axis([-1*View_Dist View_Dist -1*View_Dist View_Dist]);
clear x;x=[-1 -0.4142 0 0.4142 1]*View_Dist;
set(gca,'XTick',x);
ticks = atan(x/View_Dist)/pi*180;
set(gca,'XTickLabel', ticks);
...
% buff=sprintf(sprint_txt, FILEName, pars(1), pars(2), atan(pars(4)/View_Dist)/pi*180, atan(pars(6)/View_Dist)/pi*180, atan(pars(3)/View_Dist)/pi*180,atan(pars(5)/View_Dist)/pi*180, rf_xctr, rf_yctr, R_Value, P_Value, numXpatches, numYpatches,Dist_p);
%buff=sprintf(sprint_txt, FILE, pars(1), pars(2), atan(2.35*pars(4)/View_Dist)/pi*180, atan(2.35*pars(6)/View_Dist)/pi*180, atan(pars(3)/View_Dist)/pi*180, atan(pars(5)/View_Dist)/pi*180, rf_xctr, rf_yctr, R_Value, P_Value, numXpatches, numYpatches,Dist_p);
buff=sprintf(sprint_txt, FILE, pars(1), pars(2), pars(3),pars(4),pars(5),pars(6), View_Dist,atan(2.35*pars(4)/View_Dist)/pi*180, atan(2.35*pars(6)/View_Dist)/pi*180, atan(pars(3)/View_Dist)/pi*180,atan(pars(5)/View_Dist)/pi*180,atan(sqrt(pars(3).^2+pars(5).^2)/View_Dist)/pi*180,R_Value, P_Value, numXpatches, numYpatches,Dist_p);
% % outfile =['Z:\Users\Aihua\Cluster_Analysis\ReverseCorrelation\ReverseCorr.dat'];
% % outfile =[BASE_PATH 'ProtocolSpecific\MOOG\Reverse Correlation\ReverseCorrelation_x10.dat'];
outfile =[BASE_PATH 'ProtocolSpecific\MOOG\Reverse Correlation\ReverseCorrelation_MT.dat'];
% % outfile=['C:\Aihua\z_TempOutputs\ReverseCorrelation_MT.dat'];
%outfile=['F:\Syed_Analysis\ReverseCorrelation_MT.dat'];
printflag = 0;
if (exist(outfile, 'file') == 0)    %file does not yet exist
    printflag = 1;
end
fid = fopen(outfile, 'a');
if (printflag)
    %fprintf(fid, 'FILE\t         Base_Rate\t Amplitude\t FWHMx\t FWHMy\t Fitted_CTRx\t Fitted_CTRy\t RF_XCTR\t RF_YCTR\t R_Value_Gaussinfitting\t P_Value_Gaussianfitting\t numXpatches\t numYpatches\t Dist_p\t');
