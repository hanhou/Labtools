...
%%%%%     Aihua's MST use "z_gauss = zeros(length(x_interp),length(y_interp))
%%%%%     Katsu"s MT use ......                                              +min(min(cplot_wrapped));
% z_gauss = zeros(length(x_interp), length(y_interp));
z_gauss = zeros(length(x_interp), length(y_interp))+min(min(cplot_wrapped));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5%%%%
...
%%%%%%%%%%%or another way is multiple x10 fro MT low frequency cell
%%%%%%%%%%%%%%%%%%%%%%%% useually commented
% cplot_wrapped=cplot_wrapped*10;% If FR is too low to calculate model *10
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
for Xpatch=1:numXpatches
    for Ypatch=1:numYpatches       
...
outfile=['F:\Syed Analysis\ReverseCorrelation_MT.dat'];
printflag = 0;
if (exist(outfile, 'file') == 0)    %file does not yet exist
    printflag = 1;
end
...
if (printflag)
    ...
    fprintf(fid, 'FILE\t         Base Rate\t Amplitude\t XCenter_cm\t SigmaX_cm\t YCenter_cm\t SigmaY_cm\t ViewDistance\t FWHMx\t FWHMy\t Fitted_CTRx\t Fitted_CTRy\t Essentricity\t R_Value\t P_Value\t numXpatches\t numYpatches\t Dist_p\t');
    fprintf(fid, '\r\n');
end
fprintf(fid, '%s', buff);
fprintf(fid, '\r\n');
