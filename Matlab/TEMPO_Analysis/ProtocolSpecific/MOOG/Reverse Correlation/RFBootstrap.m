function RFBootstrap(data, Protocol, Analysis, SpikeChan, SpikeChan2, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);ProtocolDefs;
...
%OK. The basci strategy here is to iterate through every patch and use the
...
for Xpatch=1:numXpatches
    for Ypatch=1:numYpatches
        ...
        figure(5);subplot(numXpatches,numYpatches, Xpatch + (Ypatch-1)*numXpatches);plot(Response,FitData,'*');

        %Linear Regression
        [p1 mu]=polyfit(Response,FitData,1);
        x2 = 0:.1*max(xDirection):max(xDirection);  
        y2=polyval(p1,x2); hold on;plot(x2,y2,'g'); axis([0 6 0 6]);
...
%             Response=Response./DirectionNumber;
            ...
            [Azi(Xpatch,Ypatch,DelayIndex), Ele(Xpatch,Ypatch,DelayIndex), Amp(Xpatch,Ypatch,DelayIndex)] = vectorsum(Response);
        ...
        figure(7);subplot(numXpatches, numYpatches, Xpatch + (Ypatch-1)*numXpatches);hist(r_f(Xpatch,Ypatch,:),x_bin);axis([0.6 1 0 50]);title('r Variance');
...
for i=1:1000
    ...
    Distance_Permute(i)=DistanceEvaluate(permutematrix);
end
