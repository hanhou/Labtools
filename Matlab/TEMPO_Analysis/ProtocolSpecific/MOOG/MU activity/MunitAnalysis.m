...
% buff = sprintf('%s\t %4.1f\t   %4.1f\t', ...
%      FILE, BegTrial, EndTrial );
% outfile = ['Z:\LabTools\Matlab\TEMPO_Analysis\ProtocolSpecific\MOOG\MU activity\MU_SU.dat'];
%  printflag = 0;
...
% fid = fopen(outfile, 'a');
% if (printflag)
% fprintf(fid, 'FILE\t BegTrial\t EndTrial\t');
%     fprintf(fid, '\r\n');
% end
% fprintf(fid, '%s', buff);
% fprintf(fid, '\r\n');
...
% FileName0=[Directory,'\',FILE(1:end-4),'_corrSep.fig'];figure(FigureIndex); saveas(gcf,FileName0,'fig');
...
% %OutPath=['Z:\Users\Aihua\Cluster_Analysis\Rotation\'];
% cd(OutPath);
% if (exist([save_name '.mat'], 'file') == 0)    %file does not yet exist 
%     eval(['save ' OutPath save_name spacer dum_name]);   
% else 
...
%---------------------------------------------------------------------------------------
%save the figures
Directory='Z:\Users\Aihua\Cluster_Analysis\Rotation\example_figure_new';
%Directory=[OutPath '\figure'];
