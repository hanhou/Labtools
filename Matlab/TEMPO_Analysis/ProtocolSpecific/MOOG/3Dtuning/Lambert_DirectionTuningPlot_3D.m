...
%--	YONG, 6/27/03
%-----------------------------------------------------------------------------------------------------------------------
function Lambert_DirectionTuningPlot_3D(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
h_title{1}='Velocity';
h_title{2}='Accleration';
h_title{3}='Mean Firing Rate';
...
for j = 1:40
    if sum(count_y{plot_col(j), plot_row(j),current_stim}) == 0
        is_empty(j) = 1;
        count_y{plot_col(j), plot_row(j),current_stim} = zeros(1, 100);
    end
...
[pref_az_mfr pref_el_mfr mfr_pref_amp] = vectorsum(resp{current_stim});
...
% p_ddi_vel = t_ddi_vel/1000;
% t_ddi_acc = length(find(DDI_acc_rand>DDI_acc));
% p_ddi_acc = t_ddi_acc/1000;
...
%     errorbar(x_elevation,y_elevation_mean,y_elevation_ste,'k-');% Katsu for paper settle for m3c294
    ...
    for i=1:(length(unique_azimuth) )
        y_azimuth_mean(1,i)=mean(resp_mat_tran(k,4,i));
        if k ==1
    ...
    y_azimuth_mean(1,9) = mean(resp_mat_tran(k,4,1));
    y_azimuth_mean(1,9) = mean(resp_mat_tran(k,4,1));
%    for i=1:(length(unique_azimuth) )
...
% text(10,3,'Protocol         p-ANOVA    p-ANOVA-vel    p-ANOVA-acc    Azi-mfr    Ele-mfr    Azi-vel    Ele-vel    Azi-acc    Ele-acc');
% 
% h_text= [ '     ' num2str(P_anova(1), 4) '              ' num2str(P_anova_vel, 4) '          ' num2str(P_anova_acc, 4) '          ' num2str(pref_az_mfr, 4) '     ' num2str(pref_el_mfr, 4) '      ' num2str(pref_az_vel, 4) '      ' num2str(pref_el_vel, 4) '      ' num2str(pref_az_acc, 4) '      ' num2str(pref_el_acc, 4)];
...
% outfile = ['Z:\Users\Anuk\fit_analysis\data\newdata\contour_correlation_trans.dat'];
% fid = fopen(outfile, 'a');
% fprintf(fid, '%s', buff);
% fprintf(fid, '\r\n');
% fclose(fid);