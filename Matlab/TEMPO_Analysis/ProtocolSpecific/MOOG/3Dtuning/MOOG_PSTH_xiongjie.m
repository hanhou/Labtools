function MOOG_PSTH_xiongjie(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol);
 
addpath('Z:\Users\Aihua\PSTHAnalysis');
addpath('Z:\Users\Xiongjie\mfiles');
OutputPath=['Z:\Users\Xiongjie\TemporalOuput\'];
...
PSTHEnlargeWhole_xiongjie(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol,OutputPath);

% dlmwrite('BegTrial.txt',BegTrial);
% dlmwrite('EndTrial.txt',EndTrial);
% dlmwrite('StartCode.txt',StartCode);
% dlmwrite('StopCode.txt',StopCode);
% 
% % Path_Defs;
...
% %                 ylim(AX(1),[max(max_count(k,:))/2,max(max_count(k,:))]);
