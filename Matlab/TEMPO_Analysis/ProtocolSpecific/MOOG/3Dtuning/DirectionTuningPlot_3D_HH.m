...
%--	YONG, 6/27/03  
%-----------------------------------------------------------------------------------------------------------------------
function DirectionTuningPlot_3D_HH(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, batch_flag);
...
% add for 1d + up adn down
if   length(unique_elevation) == 3
    DirectionTuningPlot_3D_10(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
    return;
end
...
% if ( k==2 )
%  caxis([40, 60]); %changes the scale so that they can be the same for vis and vest.
% end
    colorbar;
...
% figure(2);saveas(gcf,['Z:\Users\Aihua\Temporal_OutputData\' FILE(1:end-4) '.png'],'png')
% % figure(2);saveas(gcf,['C:\Aihua\z_TempOutputs\figures\' FILE(1:end-4) '_n' num2str(SpikeChan-4) '.png'],'png')
% saveas(gcf,['Z:\Users\Aihua\Temporal_OutputData\' FILE(1:end-4) '.fig'])
% close (2);
%SaveTrials(FILE,BegTrial,EndTrial,repetitions)
