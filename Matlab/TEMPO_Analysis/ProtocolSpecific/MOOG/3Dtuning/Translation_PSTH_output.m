...
function Translation_PSTH_output(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol, Analysis);

Path_Defs;
ProtocolDefs; %contains protocol specific keywords - 1/4/01 BJP

%get the column of values for azimuth and elevation and stim_type
% temp_azimuth = data.moog_params(ROT_AZIMUTH,:,MOOG);% This is rotation
...
%                 count_y_norm{i,j,k}=count_y{i,j,k} / max_count_stim{2};%k=2, visual is normalization
                count_y_norm{i,j,k}=count_y{i,j,k} / max_count_stim{1};%k=1, vestibular signal is normalization 21/9/2007 AB
...
% define figure
figure(2);
set(2,'Position', [5,5 1000,700], 'Name', '3D Translation Tuning');
orient landscape;
title(FILE);
axis off;
...
%             ylim([0,max_count/max_count_stim{2}]);%----normalized by visual (k=2) stimulus
            ylim([0,max_count/max_count_stim{1}]);%----normalized by vestibular (k=1) stimulus AB 21/9/2007
...
% % outfile = [BASE_PATH 'ProtocolSpecific\MOOG\3dtuning\Peak_Trans_PSTH_output_Katsu.dat'];
% outfile = ['Z:\Users\Ayanna\tempo_output\translation_PSTH_output_Ayanna.dat']; %saves data to my z:\users folder ab 21 Sept. 2007.
% 
% printflag = 0;
