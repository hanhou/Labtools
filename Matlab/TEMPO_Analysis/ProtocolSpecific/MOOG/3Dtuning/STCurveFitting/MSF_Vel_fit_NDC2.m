function [space_fit,vect_calculated r_squared,Current_ci,cor] = MSF_Vel_fit_NDC2(spacetime_data,timestep,allow_negative)
...
if allow_negative
    LB = [0 -1.5*(max_val-min_val) 0.001 (0*45*pi/180) 0 0 ];   %     lower bounds
    UB = [1.35*max_val 1.5*(max_val-min_val) 10 (8*45*pi/180) 2.5 6 ];   % upper bounds
else
    LB = [0 0 0.001 (0*45*pi/180) 0 0 ];   % lower bounds
    UB = [1.35*max_val 1.5*(max_val-min_val) 10 (8*45*pi/180) 2.5 6 ];   % upper bounds
end
