function [chi2, chiP] = Chi2_Test_3D(xtdata, datay, funcname, params, num_free_params)
...
datax=reshape(xdata,1,size(xdata,1)*size(xdata,2));
datat=reshape(tdata,1,size(tdata,1)*size(tdata,2));
...
uniquex = munique(datax);%uniquex = munique(datax);

%compute function values at uniquex
global model_use;
funccall = [funcname '(params,xtdata)'];
fity = eval(funccall);
fity=reshape(fity,1,size(fity,1)*size(fity,2));
fity(fity < 0) = 0;
...
std_err = []; std_dev = []; temp = []; sd_zero_count = 0;

meandata=mean(datay);
std_err=std(datay)/sqrt(size(datay,1));
std_dev=std(datay);
...
%      if(std_dev(i)<0.01)%
    if(std_dev(i)==0)
        ...
        temp(i) = (meandata(i) - fity(i))^2 / std_dev(i)^2;
    end
end
