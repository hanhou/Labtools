function CurveFit=CurveFitting_Plot_VelAcc_NDC2(FILE,Plane,Azi_3D,Ele_3D,Azi_temp,Ele_temp,Step,StepMatrix,StdMatrix,SpikeCount_Trial,p_peak,Value_peak,TimeIndex_peak,p_trough,Value_trough,TimeIndex_trough)
...
contourf(XAzi,Ytime,spacefit_VelAcc_NDC2',10);%caxis([0 120]);
colorbar;    
set(gca, 'xtick', [] );set(gca, 'XTickMode','manual'); 
set(gca, 'xtick',[0:90:360]);set(gca, 'xticklabel','0|90|180|270|360');
set(gca, 'ytick', [] ); set(gca, 'YTickMode','manual');
set(gca, 'ytick',[0:0.5:max(t)]);
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
ylabel('Time (sec)');title('model: VelAcc (Without DC2)');%xlabel('Azimuth, X (deg)');  

error_surf = spacetime_data - spacefit_VelAcc_NDC2;
err_VelAcc_NDC2 = cosnlin_err_NDC2(vect_VelAcc_NDC2);
...
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
%axis off;
title({[ 'Err: ' num2str(err_VelAcc_NDC2, '%0.2f') ]},  'FontSize', 10);
...
set(gca, 'ytick',[0:0.5:max(t)]);
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
ylabel('Time (sec)');title('model: VelAcc (with DC2)');%xlabel('Azimuth, X (deg)');  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
out=[];
out = 'CurveFitting:          R0 |  Amp  |  n  | muAzi |  muT  | sigmaT | wVel  |ThetaAcc | DC2  '; 
out = strvcat(out, sprintf('------------------------------------------------------------------------------------------------------------'));    
clear OutputValue;OutputValue=vect_VelAcc_NDC2;
OutputValue(4)=OutputValue(4)*180/pi;
OutputValue(8)=OutputValue(8)*180/pi;
out=strvcat(out, sprintf('VelAcc Model(No DC2):   %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f |%6.3f | %6.3f', OutputValue));

clear OutputValue;OutputValue=[vect_VelAcc(1:6) vect_VelAcc(8:9) vect_VelAcc(7)];
OutputValue(4)=OutputValue(4)*180/pi;
OutputValue(8)=OutputValue(8)*180/pi;
out=strvcat(out, sprintf('VelAcc Model(With DC2): %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f |%6.3f | %6.3f  | %6.3f ', OutputValue));
...
out=[];
out = 'Goodness of fit: r2(VelAcc/WithoutDC2) | r2(VelAcc/WithDC2) '; 
out = strvcat(out, sprintf('--------------------------------------------------------------------------------------------------------'));    
clear OutputValue;OutputValue=[r_squared_VelAcc_NDC2 r_squared_VelAcc];
...
clear model_VelAcc_NDC2; model_VelAcc_NDC2=zeros(length(Azi_temp),length(x_time));
model_VelAcc_NDC2(:,1:StartIndex-1) = NaN; model_VelAcc_NDC2(:,StartIndex:length(x_time))=spacefit_VelAcc_NDC2(1:8,:);
...
for i=1:length(Azi_temp)
    ...
    plot(x_time,model_VelAcc(i,:),'r','LineWidth',2); hold on;   
    plot(x_time,model_VelAcc_NDC2(i,:),'g','LineWidth',2); hold on;  
