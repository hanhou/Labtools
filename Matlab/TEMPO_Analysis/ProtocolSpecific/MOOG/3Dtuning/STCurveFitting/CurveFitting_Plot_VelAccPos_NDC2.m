function CurveFit=CurveFitting_Plot_VelAccPos_NDC2(FILE,Plane,Azi_3D,Ele_3D,Azi_temp,Ele_temp,Step,StepMatrix,StdMatrix,SpikeCount_Trial,p_peak,Value_peak,TimeIndex_peak,p_trough,Value_trough,TimeIndex_trough)
...
model_use=2;
[spacefit_VelAcc_NDC2,vect_VelAcc_NDC2,r_squared_VelAcc_NDC2,CI_VelAcc_NDC2,cor_VelAcc_NDC2] = MSF_VelAcc_fit_NDC2(spacetime_data, vect_Vel_NDC2,Step*0.001,allow_negative);
model_use=3;
[spacefit_VelAccPos_NDC2,vect_VelAccPos_NDC2,r_squared_VelAccPos_NDC2,CI_VelAccPos_NDC2,cor_VelAccPos_NDC2] = MSF_VelAccPos_fit_NDC2(spacetime_data, vect_VelAcc_NDC2,Step*0.001,allow_negative);

figure(FigureIndex);axes('position',[0.05 0.53 0.17 0.15]);
contourf(XAzi,Ytime,spacefit_VelAccPos_NDC2',10);%caxis([0 120]);
colorbar;    
set(gca, 'xtick', [] );set(gca, 'XTickMode','manual'); 
set(gca, 'xtick',[0:90:360]);set(gca, 'xticklabel','0|90|180|270|360');
set(gca, 'ytick', [] ); set(gca, 'YTickMode','manual');
set(gca, 'ytick',[0:0.5:max(t)]);
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
ylabel('Time (sec)');title('model: VelAccPos (Without DC2)');%xlabel('Azimuth, X (deg)');  

error_surf = spacetime_data - spacefit_VelAccPos_NDC2;
err_VelAccPos_NDC2 = cosnlin_err_NDC2(vect_VelAccPos_NDC2);
...
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
%axis off;
title({[ 'Err: ' num2str(err_VelAccPos_NDC2, '%0.2f') ]},  'FontSize', 10);
...
contourf(XAzi,Ytime,spacefit_VelAccPos');%caxis([0 120]);
colorbar;    
set(gca, 'xtick', [] );set(gca, 'XTickMode','manual'); 
set(gca, 'xtick',[0:90:360]);set(gca, 'xticklabel','0|90|180|270|360');
set(gca, 'ytick', [] ); set(gca, 'YTickMode','manual');
set(gca, 'ytick',[0:0.5:max(t)]);
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
ylabel('Time (sec)');title('model: VelAccPos (with DC2)');%xlabel('Azimuth, X (deg)');  
...
set(gca, 'yticklabel','0|0.5|1|1.5|2|2.5');% set(gca, 'ydir','reverse');
% axis off;
title({[ 'Err: ' num2str(err_VelAccPos, '%0.2f') ]},  'FontSize', 10);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
out=[];
out = 'CurveFitting:   R0 |  Amp  |  n  | muAzi |  muT  | sigmaT | wVel  |ThetaAcc | wAcc | ThetaPos | wPos  |DC2  '; 
out = strvcat(out, sprintf('------------------------------------------------------------------------------------------------------------'));    
clear OutputValue;OutputValue=[vect_VelAccPos_NDC2(1:8) 1-vect_VelAccPos_NDC2(7) vect_VelAccPos_NDC2(9:10)];
OutputValue(4)=OutputValue(4)*180/pi;
OutputValue(8)=OutputValue(8)*180/pi;
OutputValue(10)=OutputValue(10)*180/pi;
out=strvcat(out, sprintf('VAP(NoDC2):   %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f |%6.3f | %6.2f  |%6.3f|  %6.2f  |%6.3f', OutputValue));

clear OutputValue;OutputValue=[vect_VelAccPos(1:6) vect_VelAccPos(8:9) 1-vect_VelAccPos(8) vect_VelAccPos(10:11) vect_VelAccPos(7)];
OutputValue(4)=OutputValue(4)*180/pi;
OutputValue(8)=OutputValue(8)*180/pi;
OutputValue(10)=OutputValue(10)*180/pi;
out=strvcat(out, sprintf('VAP(WithDC2): %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f |%6.3f | %6.2f  |%6.3f|  %6.2f  |%6.3f |%6.3f', OutputValue));
...
out=[];
out = 'Goodness of fit: r2(VelAccPos/NoDC2) | r2(VelAccPos/WithDC2) '; 
out = strvcat(out, sprintf('--------------------------------------------------------------------------------------------------------'));    
clear OutputValue;OutputValue=[r_squared_VelAccPos_NDC2 r_squared_VelAccPos];
...
clear model_VelAccPos_NDC2; model_VelAccPos_NDC2=zeros(length(Azi_temp),length(x_time));
model_VelAccPos_NDC2(:,1:StartIndex-1) = NaN; model_VelAccPos_NDC2(:,StartIndex:length(x_time))=spacefit_VelAccPos_NDC2(1:8,:);
...
for i=1:length(Azi_temp)
    ...
    plot(x_time,model_VelAccPos(i,:),'r','LineWidth',2); hold on;   
    plot(x_time,model_VelAccPos_NDC2(i,:),'g','LineWidth',2); hold on;   
...
CurveFit.spacefit_VelAcc_NDC2=spacefit_VelAcc_NDC2;
CurveFit.vect_VelAcc_NDC2=vect_VelAcc_NDC2;
CurveFit.CI_VelAcc_NDC2=CI_VelAcc_NDC2;
CurveFit.cor_VelAcc_NDC2=cor_VelAcc_NDC2;
% CurveFit.err_VelAcc_NDC2=err_VelAcc_NDC2;
CurveFit.r_squared_VelAcc_NDC2=r_squared_VelAcc_NDC2;

%Vel+Acc+Pos model
CurveFit.spacefit_VelAccPos=spacefit_VelAccPos;
...
CurveFit.spacefit_VelAccPos_NDC2=spacefit_VelAccPos_NDC2;
CurveFit.vect_VelAccPos_NDC2=vect_VelAccPos_NDC2;
CurveFit.CI_VelAccPos_NDC2=CI_VelAccPos_NDC2;
CurveFit.cor_VelAccPos_NDC2=cor_VelAccPos_NDC2;
CurveFit.err_VelAccPos_NDC2=err_VelAccPos_NDC2;
CurveFit.r_squared_VelAccPos_NDC2=r_squared_VelAccPos_NDC2;
