...
%--	YONG, 6/27/03
%-----------------------------------------------------------------------------------------------------------------------
function Frontal_parralel_plane(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
% end
% if ( k==2 )
%  caxis([60, 120]); %changes the scale so that they can be the same for vis and vest.
% end
    colorbar;
...
% outfile = [BASE_PATH 'ProtocolSpecific\MOOG\3Dtuning\DirectionTuningSum_Iolaus_mapping.dat']; % Chris
