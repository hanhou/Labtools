...
function Temporal_Analysis_Translation(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, StartEventBin, StopEventBin, PATH, FILE, Protocol);
Path_Defs;
...
% end
% spon_count = floor(sum(temp_spike_rates(null_trials))/20);     % What is this? (crf, 2/2007)
...
for k=1:length(unique_condition_num)
    for j=1:length(unique_elevation)
        for i=1: length(unique_azimuth)
            select = logical( (azimuth==unique_azimuth(i)) & (elevation==unique_elevation(j)) & (condition_num==k) );            
            % get rid off -90 and 90 cases
            if (sum(select) > 0)
                act_found = find( select==1 );
                for repeat=1:length(act_found) 
...
% % Following is the old Anova code... not fully accurate? ignores partial
% % trials.. and dft_velocity_all
% % % % 
