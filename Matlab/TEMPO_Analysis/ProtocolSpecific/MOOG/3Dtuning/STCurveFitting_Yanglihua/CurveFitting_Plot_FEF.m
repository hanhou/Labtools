function CurveFit=CurveFitting_Plot_FEF(FILE,Plane,Azi_3D,Ele_3D,Azi_temp,Ele_temp,Step,x_time,x_stop,StepMatrix,StdMatrix,SpikeCount_Trial,p_peak,Value_peak,TimeIndex_peak,p_trough,Value_trough,TimeIndex_trough)
...
%     else
%     end    
%     clear tempdata; tempdata=[SpikeCount_Trial{1,RowIndex,ColIndex}]';
% %     size(SpikeCount_Trial)
% %     size(tempdata)   
%     if i>1
...
%get the space time data for different plane (only for visual condition)
for i=1:length(Azi_temp) 
    clear RowIndex ColIndex; [RowIndex, ColIndex]=find(AziGrid==Azi_temp(i) & EleGrid==Ele_temp(i));
    PSTH_tempPlane(i,:)=StepMatrix{2}(RowIndex,ColIndex,:);%PSTH_tempPlane(i,:)=StepMatrix{1}(RowIndex,ColIndex,:)*1000/Step;  
    p_peak_tempPlane(i,1)=p_peak{2}(RowIndex,ColIndex);
    p_trough_tempPlane(i,1)=p_trough{2}(RowIndex,ColIndex);
    PeakValue_tempPlane(i,1)=Value_peak{2}(RowIndex,ColIndex);
    TroughValue_tempPlane(i,1)=Value_trough{2}(RowIndex,ColIndex);
    PeakTimeIndex(i,1)=TimeIndex_peak{2}(RowIndex,ColIndex);
    if x_time(PeakTimeIndex)>2
        p_peak_tempPlane(i,1)=NaN;
    else
    end
    TroughTimeIndex(i,1)=TimeIndex_trough{2}(RowIndex,ColIndex); 
    if x_time(TroughTimeIndex(i,1))>2
        p_trough_tempPlane(i,1)=NaN;
    else
    end    
    clear tempdata; tempdata=[SpikeCount_Trial{2,RowIndex,ColIndex}]';
%     size(SpikeCount_Trial)
%     size(tempdata)   
    if i>1
        tempdata0=zeros(size(spacetime_data_trial,1),size(spacetime_data_trial,3));
