function CurveFit=CurveFitting_Plot_NDC2_All(FILE,Plane,Azi_3D,Ele_3D,Azi_temp,Ele_temp,Step,StepMatrix,StdMatrix,SpikeCount_Trial,p_peak,Value_peak,TimeIndex_peak,p_trough,Value_trough,TimeIndex_trough)
...
global model_use
model_use=1;
[spacefit_Vel,vect_Vel,r_squared_Vel,CI_Vel,cor_Vel] = MSF_Vel_fit_NDC2(spacetime_data,Step*0.001,allow_negative);
...
err_Vel = cosnlin_err_NDC2(vect_Vel);
...
xtdata = [xdata;tdata];
model_use=1;
[chi2_Vel, chi2P_Vel] = Chi2_Test_3D(xtdata, spacetime_data_re, 'funccosnlin_NDC2', vect_Vel, length(vect_Vel));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Model 2: Velocity + Acceleration
model_use=2;
[spacefit_VelAcc,vect_VelAcc,r_squared_VelAcc,CI_VelAcc,cor_VelAcc] = MSF_VelAcc_fit_NDC2(spacetime_data, vect_Vel,Step*0.001,allow_negative);
...
err_VelAcc = cosnlin_err_NDC2(vect_VelAcc);
...
% Do chi-square goodness of fit test 
[chi2_VelAcc, chi2P_VelAcc] = Chi2_Test_3D(xtdata, spacetime_data_re, 'funccosnlin_NDC2', vect_VelAcc, length(vect_VelAcc));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Model 3: Velocity + Acceleration + Position
model_use=3;
[spacefit_VelAccPos,vect_VelAccPos,r_squared_VelAccPos,CI_VelAccPos,cor_VelAccPos] = MSF_VelAccPos_fit_NDC2(spacetime_data, vect_VelAcc,Step*0.001,allow_negative);
...
err_VelAccPos = cosnlin_err_NDC2(vect_VelAccPos);
...
% mode 3
[chi2_VelAccPos, chi2P_VelAccPos] = Chi2_Test_3D(xtdata, spacetime_data_re, 'funccosnlin_NDC2', vect_VelAccPos, length(vect_VelAccPos));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
out=[];
out = 'CurveFitting:    R0 |  Amp  |  n  | muAzi |  muT  | sigmaT | wVel  |ThetaAcc|  wAcc | ThetaPos | wPos'; 
out = strvcat(out, sprintf('------------------------------------------------------------------------------------------------------------'));    
...
out=strvcat(out, sprintf('Vel Model:     %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f ', OutputValue));

clear OutputValue;OutputValue=vect_VelAcc;
OutputValue(4)=OutputValue(4)*180/pi;
OutputValue(8)=OutputValue(8)*180/pi;
out=strvcat(out, sprintf('Vel+Acc Model: %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f |%6.3f | %6.1f  ', OutputValue));

clear OutputValue; OutputValue=[vect_VelAccPos(1:8) 1-vect_VelAccPos(7) vect_VelAccPos(9:10)];
OutputValue(4)=OutputValue(4)*180/pi;
OutputValue(8)=OutputValue(8)*180/pi;
OutputValue(10)=OutputValue(10)*180/pi;
out=strvcat(out, sprintf('Vel+Acc+Pos  : %4.1f | %5.1f |%4.1f |%6.1f | %4.3f | %6.3f |%6.3f | %6.1f |%6.3f | %6.1f   |%6.3f', OutputValue)); 
