%-----------------------------------------------------------------------------------------------------------------------
%-- MPSpeedTuningCurve.m -- Plots a speed tuning curve.
%--	GCD, 1/26/00
%-- Writes out slope information to a file for MP analyses.
%-- JWN, 01/10/06
%-----------------------------------------------------------------------------------------------------------------------
...
disp(sprintf('(MPSpeedTuningCurve) Started at %s.',datestr(now,14)));
...
%         zero_resp = NaN;
%     end
%     buff = sprintf('%s\t %6.1f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.2f\t %5.1f\t %6.2f\t %6.3f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.3f\t %10.8f\t %6.3f\t %10.8f\t %6.3f\t %10.8f\t %6.4f\t %6.3f\t %6.2f\t %6.2f\t %10.7f\t %10.7f\t %10.7f\t %8.5f\t %10.8f\t %6d\t %6d\t %6.4f\t %6.4f\t', ...
...
%         0, zero_resp, pmax.x, pmax.y, pmin.x, pmin.y, null_rate, SMI, p_value, stats1(1), stats1(3), stats2(1), stats2(3), SDI, var_term, pars, chi2, chiP, length(plot_y), length(pars), fit_err, avg_var_mean);
...
%     fid = fopen(outfile, 'a');
%     if (printflag)
%         fprintf(fid, 'FILE\t\t PrDir\t PrSpd\t PrHDsp\t RFX\t RFY\t RFDiam\t ZerSpd\t ZerRsp\t MaxSpd\t MaxRsp\t MinSpd\t MinRsp\t Spont\t SpdMI\t AnovaP\t\t Rmeans\t Pmeans\t\t Rraw\t Praw\t\t SDI\t VarTrm\t R0\t K\t alpha\t\t tau\t\t expon\t\t Chi2\t\t ChiP\t\t Npts\t Npars\t FitErr\t AvgVMR\t');
%         fprintf(fid, '\r\n');
%     end
%     fprintf(fid, '%s', buff);
%     fprintf(fid, '\r\n');
...
% Prepare data for writing out to MP specific file.  JWN 011006
% Redefine raw_slope
for k=1:(length(px)-1)
    if (px(k+1)< 35.0)
        raw_slope(k) = (py(k+1)-py(k))/(px(k+1)-px(k));
    end
end
% Cherry pick some specific slopes
total_slope = (py(7)-py(2))/(px(7)-px(2));  % not actually total
used_slope = (py(6)-py(1))/(px(6)-px(1)); % range used by MP stimuli
% pmax.x and pmin.x are from above
% Write results for this cell to a common file
area = 'MT'
PATHOUT = 'Z:\Data\MOOG\Barracuda\Analysis\';
filenames = {'BOSpeedData144'};
for i = 1:1
    ...
    if (headerflag)
        fprintf(fid, 'FILE FitMax FitMin 0m8 0.5m16 0m0.5 0.5m1 1m2 2m4 4m8 8m16 16m32');
        fprintf(fid, '\r\n');
    end
    fprintf(fid,'%10s', strtok(FILE,'.'));
    fprintf(fid,' %+2.3f', pmax.x, pmin.x, used_slope, total_slope, raw_slope);
    fprintf(fid,'\r\n');
    fclose(fid);
end

disp('(MPSpeedTuningCurve) Done.');
return;