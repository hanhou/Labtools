...
%--	TU 07/21/01
%-----------------------------------------------------------------------------------------------------------------------
function GaborFit_RelDisp(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
        ...
        if(p_value(i) < 0.01) %calculate shift ratio only if tuning is significant
            ...
            for j = i+1:length(unique_hor_disp_sur) %calculate shift ratio for each other surround disparity
                if(p_value(j) < 0.01)
                    ...
                    sum_shared_params = sum(shared_param_flags);
                    ...
                    k = 1;
                    params_temp{1} = pars_shared_signif(1:6);
                    params_temp{2} = pars_shared_signif(1:6);
                    for m=1:6
                        if(shared_param_flags(m) == 0)
                            params_temp{2}(m) = pars_shared_signif(6 + k);
                            k = k + 1;
                        end
                    end

                    y_gabor_fit{1} = gaborfunc(x_interp, params_temp{1});
                    y_gabor_fit{1}(y_gabor_fit{1} < 0) = 0;
                    y_gabor_fit{2} = gaborfunc(x_interp, params_temp{2});
                    y_gabor_fit{2}(y_gabor_fit{1} < 0) = 0;
                    temp = [x_interp' y_gabor_fit{1}' y_gabor_fit{2}'];
                    ...
                    n_shared_params = length(shared_param_flags)*n_sur_disp-(n_sur_disp-1)*sum_shared_params;
                    Npts = length(spike_rates(~null_trials & ~control_trials & select_trials & hor_disp_sur_select));
                    Fseq = ( (shared_err - independent_err )/(n_indep_params-n_shared_params) ) / ( independent_err/(Npts - n_indep_params) );
                    ...
                    outstr2 = sprintf('%s %s %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f\r\n'...
                        , outstr2, FILE, data.neuron_params(RF_DIAMETER, 1), shift_ratio, Pseq, shift_ratio_paired, Pseq_paired, Pseq_gaussCtr, null_rate, params_temp{1}(1), params_temp{1}(2), params_temp{1}(3)...
                        , params_temp{1}(4), params_temp{1}(5), params_temp{1}(6), params_temp{2}(1), params_temp{2}(2), params_temp{2}(3), params_temp{2}(4), params_temp{2}(5), params_temp{2}(6));
                else
...
outfile1 = [BASE_PATH 'ProtocolSpecific\RelativeDisparity\Seq_Ftest_GCtr.dat'];
outfile2 = [BASE_PATH 'ProtocolSpecific\RelativeDisparity\Shift_ratio_summary.dat'];
outfile3 = [BASE_PATH 'ProtocolSpecific\RelativeDisparity\center_size.dat'];
...
if (printflag)
    fprintf(fid, 'File CtrSize ShiftRatio PInd ShiftRatioPaired PPaired PGaussCtr Spont Baserate1 Amplitude1 GaussCtr1 GaussSD1 Freq1 Phase1 Baserate2 Amplitude2 GaussCtr2 GaussSD2 Freq2 Phase2');
    fprintf(fid, '\r\n');
end
