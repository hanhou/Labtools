...
%     outfile = [BASE_PATH 'ProtocolSpecific\SpeedTuning\v3ASpeedTuningSummary_Gamma.dat'];
%     printflag = 0;
...
% 
%    %---------------------------------------------------------------------------------------
%     %write out raw speed tuning mean firing rate data
%     
% %     buff = sprintf('%8.5f\t', py);
% %     outfile = [BASE_PATH 'ProtocolSpecific\SpeedTuning\SpeedTuningRawRates.dat'];
% %     fid = fopen(outfile, 'a');
% %     fprintf(fid, '%s', buff);
% %     fprintf(fid, '\r\n');
% %     fclose(fid);
...
%interpolation 
%     xinterprange = linspace(0,64,100);
%     y_interp = interp1(px,py,xinterprange,'nearest');
%     x_interp = xinterprange;
%     format('long');
%     outfile = ['Z:\Users\JRB\Dora_grant_figures\speed2.txt'];
%     for i=1:length(x_interp)
%     fid = fopen(outfile, 'a');
%     x_int = x_interp(i);
%     y_int = y_interp(i);
...
%     end
%     fclose(fid);
%    outfile1 = ['Z:\Users\JRB\Dora_grant_figures\speederr2.txt'];
%     for i=1:length(px)
...
save(['Z:\Users\Yun\speed\' FILE '.mat']);
saveas(gcf,['Z:\Users\Yun\speed\' FILE '.png'],'png');
