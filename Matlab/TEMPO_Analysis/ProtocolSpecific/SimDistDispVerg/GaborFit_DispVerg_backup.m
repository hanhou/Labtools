...
if (data.eye_calib_done == 1)
    ...
    vergence_h = Leyex_positions(trials) - Reyex_positions(trials);
    vergence_v = Leyey_positions(trials) - Reyey_positions(trials);
else     
...
num_shared_fits = 4;
Base_Rate = [1 1 1 1]; Amplitude = [1 1 0 0]; Gauss_Ctr = [1 0 1 0]; Gauss_SD = [1 1 1 1]; Sine_Freq = [1 1 1 1]; Sine_Phase = [1 1 1 1];
Fit_labels = [{'ShAll'} {'Ctr'} {'Amp'} {'CtrAmp'}];
Fit_labels = [Fit_labels {'Uncons'}]; %add label for unconstrained fit performed above

for h=1:num_shared_fits    
...
for i=1:num_shared_fits
%    shared_err(i) = multi_gaborerr(pars_shared{i}); %now calculated
%    immediately after gaborfit
    n_free_fit_params(i) = n_free_fit_params(num_shared_fits+1)-(length(unique_depth_fix_real)-1)*sum(shared_param_flags{i});
end
...
for i=1:(num_shared_fits+1)   %+1 includes the unconstrained fit
...
for i=1:(num_shared_fits+1) 
    for j=1:length(unique_depth_fix_real)
        if (i==(num_shared_fits+1))
            y_gaborfit{i}{j} = gaborfunc(x_interp, pars{j});
...
if (printflag)
    ...
    for i=1:(num_shared_fits+1)
        fprintf(fid, '%s:BasRt\t %s:Amp\t %s:Ctr\t %s:Siz\t %s:Freq\t %s:Phase\t %s:FFTFr\t %s:Rsq_Means\t %s:Rsq_Raw\t',Fit_labels{i},Fit_labels{i},Fit_labels{i},Fit_labels{i},Fit_labels{i},Fit_labels{i},Fit_labels{i},Fit_labels{i},Fit_labels{i});
    end
    for i=1:num_Ftests
...
for j = 1:length(unique_depth_fix_real)
    ...
    for i=1:num_shared_fits
        outbuff{3+i} = sprintf('%7.2f\t %7.2f\t %6.3f\t %7.4f\t %6.3f\t %6.3f\t %6.3f\t %6.3f\t %6.3f\t', ...
            params{i}{j}(1), params{i}{j}(2), params{i}{j}(3), params{i}{j}(4), params{i}{j}(5), params{i}{j}(6), freq_fit{i}(j), stats1{i}{j}(1), stats2{i}{j}(1));
    end
    for i=1:num_Ftests
        outbuff{4+num_shared_fits+i} = sprintf('%7.4f\t %7.4f\t', Fseq(i), Pseq(i));
    end
