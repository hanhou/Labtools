%-----------------------------------------------------------------------------------------------------------------------
% RefitEyeVergData.m -- Plots vergence data using both the linear and
% nonlinear eye calibration fits
%-----------------------------------------------------------------------------------------------------------------------

function RefitEyeVergData(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
if (data.eye_calib_done)
    eye_positions{1} = data.eye_positions_calibrated;
else
    eye_positions{1} = data.eye_positions;
end


%load nonlinear fitting parameters and compute calibrated eye positions
[eye_positions{2} doneflag] = LoadEyeCalibration_NonLin(data, PATH, FILE);
...
for i=1:2
    ...
    verg_hor = eye_positions{i}(REYE_H, select_trials) - eye_positions{i}(LEYE_H, select_trials);
