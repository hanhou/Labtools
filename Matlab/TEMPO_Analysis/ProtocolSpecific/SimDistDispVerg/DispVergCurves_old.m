...
depth_fix_real = all_depth_fix_real(~null_trials); %excludes null trials
...
[p_near_x, p_near_y, p_near_err] = PlotTuningCurve ([near_trials(1,:)]', [near_trials(2,:)]', 'ro', 'r-', 0, 0);
%hold on;
[p_mid_x, p_mid_y, p_mid_err] = PlotTuningCurve ([mid_trials(1,:)]', [mid_trials(2,:)]', 'go', 'g-', 0, 0);
%hold on;
[p_far_x, p_far_y, p_far_err] = PlotTuningCurve ([far_trials(1,:)]', [far_trials(2,:)]', 'bo', 'b-', 0, 0);
...
plot(null_x, [near_null_rate near_null_rate], 'r--');
plot(null_x, [mid_null_rate mid_null_rate], 'g--');
plot(null_x, [far_null_rate far_null_rate], 'b--');
hold off;

yl = YLim;
YLim([0 yl(2)]);	% set the lower limit of the Y axis to zero
XLabel('Relative Depth (deg)');
YLabel('Response (spikes/sec)');
legend([near_handle, mid_handle, far_handle], '28.5cm fixation', '57cm fixation', '114cm fixation',0);
