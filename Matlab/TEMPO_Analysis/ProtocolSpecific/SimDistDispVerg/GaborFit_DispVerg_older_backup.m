...
n_indep_params = length(shared_param_flags)*length(unique_depth_fix_real);
n_shared_params = n_indep_params-(length(unique_depth_fix_real)-1)*sum(shared_param_flags);
...
% fid = fopen(outfile, 'a');
% if (printflag)
%     fprintf(fid, 'FILENAME\t PrDir\t PrSpd\t PrHDsp\t RFXCtr\t RFYCtr\t RFDiam\t FixDst\t MaxDsp\t MaxRsp\t MinDsp\t MinRsp\t AvgRsp\t Spont\t DDI  \t VarTrm\t BasRt\t Amp\t Ctr\t Siz\t Freq\t Phase\t FFTFr\t ShBasRt\t ShAmp\t ShCtr\t ShSiz\t ShFr\t ShPh\t ShFFTFr\t Fseq\t Pseq\t Peak\t Trough\t');
%     fprintf(fid, '\r\n');
%     printflag = 0;
% end
% for j = 1:length(unique_depth_fix_real)
...
%         unique_depth_fix_real(j), pmax(j).x, pmax(j).y, pmin(j).x, pmin(j).y, avg_resp(j), null_rate, DDI_index(j), var_term(j));
