    ...
    for j=1:length(unique_mag_disp)
        ...
        for k=1:length(unique_mean_disp)
            ...
            if length(unique_mean_disp) > length(unique_mag_disp)
                ...
                angle_string = sprintf(' %1.3f %3.2f %3.2f %1.3f', unique_mag_disp(k), angle_max(j), angle_min(j), p_val(i, j));
            ...
            modIndex = (max_resp(k)-min_resp(k))/(max_resp(k)-null_rate);
            
            if num_mag_disp < length(unique_mean_disp)
                stat_string = sprintf('\t%1.3f\t%1.4f\t%1.4f\t%1.4f\t%1.4f\t%1.4f\t%1.4f\t%3.2f\t%1.4f\t%1.4f\t%3.2f\t%1.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%1.3f\t%1.4f', unique_mean_disp(k), stats_mean(1), stats_mean(2), stats_mean(3), stats_raw(1), stats_raw(2), stats_raw(3), pars{k}(1), pars{k}(2), pars{k}(3), pars{k}(4), pars{k}(5), angle_max(k), max_resp(k), angle_min(k), min_resp(k), p_val(i, k), single_TDI, modIndex);
                stat_out{k+1} = stat_string;
            else
                stat_string = sprintf('\t%1.3f\t%1.4f\t%1.4f\t%1.4f\t%1.4f\t%1.4f\t%1.4f\t%3.2f\t%1.4f\t%1.4f\t%3.2f\t%1.2f\t%3.2f\t%3.2f\t%3.2f\t%3.2f\t%1.3f\t%1.4f', unique_mag_disp(j), stats_mean(1), stats_mean(2), stats_mean(3), stats_raw(1), stats_raw(2), stats_raw(3), pars{k}(1), pars{k}(2), pars{k}(3), pars{k}(4), pars{k}(5), angle_max(k), max_resp(k), angle_min(k), min_resp(k), p_val(i, j), single_TDI, modIndex);
...
if do_combo == 1
    ...
    if length(unique_mean_disp) > 1
        ...
        for j=1:length(unique_mean_disp)
            pref_out = '';
            if (p_val(i,j) < 0.01) %if significant
...
if (printme==1)
    ...
    %print grad metrics
    outfile = [PATHOUT 'slant_metrics_05_12_03.dat'];
    fid = fopen(outfile, 'a');
    if (exist(checkfile, 'file') == 0)    %file does not yet exist
