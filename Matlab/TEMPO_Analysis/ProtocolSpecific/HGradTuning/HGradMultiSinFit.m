function HGradMultiSinFit(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, PATH, FILE)
...
for i=1:length(unique_ap_size)
    ...
    if(num_ap_size >= num_mag_disp)
        ...
        %print out curves for origin
        printcurves = 1;
        if printcurves == 1
            size_array = length(x_deg);
            for go=1:length(x_deg)
                if isempty(curve_out{go})
                    curve_out{go} = '';
                end
                curve_out{go} = sprintf('%s%6.2f\t%6.2f\t\t\t\t\t\t', curve_out{go}, x_deg(go), sin{1}(go));
        ...
        for mdisp_ind = 2:length(unique_mean_disp)
            pars_short(1) = pars(index);
            pars_short(4) = pars(index+1);
            pars_short(5) = pars(index+2);
            index = index + 3;
            hold on;
            sin{mdisp_ind} = sin_exp_func(x_interp, pars_short);
            sin{mdisp_ind}(sin{mdisp_ind} < 0) = 0;
            plot(x_deg, sin{mdisp_ind}, lines{mdisp_ind});
            ...
            if printcurves == 1
                for go=1:length(x_deg)
                    ...
                    curve_out{go} = sprintf('%s%6.2f\t%6.2f\t\t\t\t\t\t', curve_out{go}, x_deg(go), sin{mdisp_ind}(go));
                    
                end
                curve_out{1}
            end
            
            %r^2 for fit w/o phase slop--------------------------------------------------------
            %calculate R^2 of mean response
            %add a column of ones to yfit to make regress happy
            y_fit_mean = sin_exp_func(x_means{mdisp_ind}, pars_short);
            ...
            %run chi^2 test on fit
            [chi2_simul(mdisp_ind), chiP_simul(mdisp_ind)] = Chi2_Test(raw{mdisp_ind}(:,1), raw{mdisp_ind}(:,2), 'sin_exp_func', pars_short, num_free_params);            
        end
        ...
        z = sin_exp_func(raw{1}(:,1),pars(1:5));
        z(z < 0) = 0;
        ...
        for mdisp_ind = 1:length(unique_mean_disp)
            ...
            if mdisp_ind > 1
                q_temp(1) = pars(index);
                q_temp(4) = pars(index+1);
                q_temp(5) = pars(index+2);
    ...
    text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;         
    line = sprintf('Amp(1) = %3.2f', pars(1));
    text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
    line = sprintf('Base(1) = %3.2f', pars(4));
    text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
    line = sprintf('Exp(1) = %1.2f', pars(5));
    text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
    line = sprintf('Freq = %1.4f', pars(2));
    text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
    line = sprintf('Phase = %1.4f', pars(3));
    text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
    ...
    if(num_ap_size >= num_mag_disp)
        ...
        for mdisp_ind = 2:length(unique_mean_disp)      
            ...
            text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;         
            line = sprintf('Amp(%1.0d) = %3.2f', mdisp_ind, pars(index));
            text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;
            line = sprintf('Base(%1.0d) = %3.2f', mdisp_ind, pars(index+1));
            text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;         
            line = sprintf('Exp(%1.0d) = %3.2f', mdisp_ind, pars(index+2));
            text(xpos,ypos,line,'FontSize',font_size);		ypos = ypos - bump_size;         
...
if printcurves == 1
    ...
    FILEOUT = [FILE(1:filesize) 'simul_curve'];
...
if (printme==1)
    ...
    outfile = [PATHOUT 'Sim_fit_mdispStats_slant_82002.dat'];
    print_label = 0;
    ...
    outfile = [PATHOUT 'Sim_fit_cellStats_slant_82002.dat'];
    print_label = 0;
