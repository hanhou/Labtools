function SurroundModel(data, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE, HDisparity, SurrMap);
...
[StartOffsetBin StopOffsetBin] = CheckTimeOffset(data, num_trials, StartCode, StopCode, StartOffset, StopOffset);
...
Total_RF_Size = SurrMap.ctrsize + SurrMap.surrsize;
...
px = HDisparity.px;
py = HDisparity.py;
perr = HDisparity.perr
uncorr_resp = HDisparity.uncorr_resp;
...
Ksurr(1:num_surround) = [1                   1                      1               1        1            1];
%initialize a matrix to hold the distance of the stimulus from the CRF center
...
plot_x = HDisparity.plot_x;
plot_y = HDisparity.plot_y;
...
%now fit a gabor to all the surround tuning curves
for i = 1:num_surround
   means = [SurrMap.px(:, i) SurrMap.py(:, i)];
   raw = [SurrMap.plot_x(i, :)' SurrMap.plot_y(i, :)'];
   fixed_param_flags = zeros(6,1); %by default, all 6 parameters will vary
   fixed_param_values = zeros(6,1); 
   [pars,freq] = gaborfit(means,raw, fixed_param_flags, fixed_param_values);
   x_interp = (px(1)): .01 : (px(length(px)));
   ...
   y_interp2(y_interp2 < 0) = 0;
   y_surr(i,:)=y_interp2;
end



%now create the tuning curves for the surround
%no_shift = pars(3);
%shift_far = pars(3) + 1;
%shift_near = pars(3) - 1;
