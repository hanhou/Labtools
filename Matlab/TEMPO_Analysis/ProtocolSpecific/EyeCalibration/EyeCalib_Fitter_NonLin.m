function [horiz_pars, vert_pars] = EyeCalib_Fitter_NonLin(horiz_raw, vert_raw, horiz_desired, vert_desired, H_lower_bounds, H_upper_bounds, V_lower_bounds, V_upper_bounds)
...
a(3) = 0.0; % 2nd order on iso axis
a(4) = 0.0; % gain on ortho axis
a(5) = 0.0; % 2nd order on ortho axis
...
horiz_pars = fmincon('EyeCalib_Func_NonLin',a,A,B,Aeq,Beq,LB,UB, NONLCON, OPTIONS);

a(1) = 0;  %offset
a(2) = 0.0; % gain on iso axis
a(3) = 0.0; % 2nd order on iso axis
a(4) = 1.0; % gain on ortho axis
a(5) = 0.0; % 2nd order on ortho axis
...
LB=V_lower_bounds;
UB=V_upper_bounds;
vert_pars = fmincon('EyeCalib_Func_NonLin',a,A,B,Aeq,Beq,LB,UB, NONLCON, OPTIONS);
