function [caldata, doneflag] = LoadEyeCalibration(data, PATH, FILE);
...
in_name = [PATHIN file_root_name '_linear_eye_calib.mat'];
...
if exist(in_name)
    ...
    [clh, clv] = ComputeCalibratedEyePosn(data.eye_positions(LEYE_H,:), data.eye_positions(LEYE_V,:), Pars.M(:, LEYE_H)', Pars.M(:, LEYE_V)' );  
    caldata(LEYE_H, :) = clh;
    caldata(LEYE_V, :) = clv;

    %compute and store the calibrated positions for the right eye
    [crh, crv] = ComputeCalibratedEyePosn(data.eye_positions(REYE_H,:), data.eye_positions(REYE_V,:), Pars.M(:, REYE_H)', Pars.M(:, REYE_V)' );  
    caldata(REYE_H, :) = crh;
    caldata(REYE_V, :) = crv;
    doneflag = 1;
