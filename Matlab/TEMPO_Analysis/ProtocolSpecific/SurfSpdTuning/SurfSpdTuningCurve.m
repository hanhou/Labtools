...
%--	GCD, 1/26/00
%-----------------------------------------------------------------------------------------------------------------------
function SurfSpdTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
matfileid = [BASE_PATH 'ProtocolSpecific\SpeedTuning\dprime_population.mat'];
...
%     fclose(proffid);
    ...
    outfile = [PATHOUT 'SurfSpd_12.09.04.dat'];
    ...
    fid = fopen(outfile, 'a');
    if (printflag)
        fprintf(fid, 'FILE\t\t PrDir\t PrSpd\t PrHDsp\t RFX\t RFY\t RFDiam\t ZerSpd\t MaxSpd\t MaxRsp\t MinSpd\t MinRsp\t Spont\t SpdMI\t AnovaP\t\t Rmeans\t Pmeans\t\t Rraw\t Praw\t\t SDI\t VarTrm\t R0\t K\t alpha\t\t tau\t\t expon\t\t Chi2\t\t ChiP\t\t Npts\t Npars\t FitErr\t AvgVMR\t');
        fprintf(fid, '\r\n');
    end
    fprintf(fid, '%s', buff);
    fprintf(fid, '\r\n');
