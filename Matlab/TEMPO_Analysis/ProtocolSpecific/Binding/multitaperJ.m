function varargout = multitaperJ(x,y, varargin);
...
Sxx = zeros(params.nfft, 1);
Sxy = zeros(params.nfft, 1);
Syy = zeros(params.nfft, 1);
Jxx = zeros(params.nfft, 2*params.NW-1);
Jyy = zeros(params.nfft, 2*params.NW-1);
Jxy = zeros(params.nfft, 2*params.NW-1);

[Sxx,k,Jxx] = mtm_spectrum(x,x, params);
[Syy,k,Jyy] = mtm_spectrum(y,y, params);
[Sxy,k,Jxy] = mtm_spectrum(x,y, params);
...
%extracted from computepsd
%Create a copy of Sxx because we want to return the unscaled (half or whole) power spectrum
Jxx_unscaled = Jxx; % Make sure Sxx_unscale, Sxx and Pxx are all columns
Jyy_unscaled = Jyy; % Make sure Sxx_unscale, Sxx and Pxx are all columns
Jxy_unscaled = Jxy; % Make sure Sxx_unscale, Sxx and Pxx are all columns
w = w(:); % Make sure we always returns a column vector for frequency

% Generate the one-sided spectrum if so wanted
if strcmp(params.range,'onesided'),
   Jxx_unscaled = Jxx(select, :); % Take only [0,pi] or [0,pi)
   Jyy_unscaled = Jyy(select, :); % Take only [0,pi] or [0,pi)
   Jxy_unscaled = Jxy(select, :); % Take only [0,pi] or [0,pi)
   Jxx = [Jxx_unscaled(1,:); Jxx_unscaled(2:end-1,:); Jxx_unscaled(end,:)]; % This is the one-sided spectrum [Power]
   Jyy = [Jyy_unscaled(1,:); Jyy_unscaled(2:end-1,:); Jyy_unscaled(end,:)]; % This is the one-sided spectrum [Power]
   Jxy = [Jxy_unscaled(1,:); Jxy_unscaled(2:end-1,:); Jxy_unscaled(end,:)]; % This is the one-sided spectrum [Power]
end
...
switch nargout,
    ...
    case 8, varargout = {Sxx,Syy,Sxy,f, Jxx(find(f<cutoff_freq),:), Jyy(find(f<cutoff_freq),:),Jxy(find(f<cutoff_freq),:),f(find(f<cutoff_freq),:)};
end
...
%----------------------------------------------------------------------
function [S,k, j] = mtm_spectrum(x, y, params);
...
if N<=nfft
    ...
    %Sk=(fft(E(:,1:k).*x(:,ones(1,k)),nfft) - mean(x).*H ) .* (conj(fft(E(:,1:k).*y(:,ones(1,k)),nfft)) - mean(x).*H  );
   ...
   %Sk=(czt(E(:,1:k).*x(:,ones(1,k)),nfft)  - mean(x).*H     ).* (conj(czt(E(:,1:k).*y(:,ones(1,k)),nfft))  - mean(x).*H ); 
