...
%-----------------------------------------------------------------------------------------------------------------------
function BindingTempFreqTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
temp_freq = data.obj_params(OBJ_FREQ,:,1);

unique_freq = munique(temp_freq);
...
trials = 1:length(temp_freq);		% a vector of trial indices
...
subplot(2, 1, 2);
	...
	plot_x = temp_freq(~null_trials & select_trials );
...
Xlim([0 max(x_interp)]);
XLabel('Temporal Frequency (c/deg)');
