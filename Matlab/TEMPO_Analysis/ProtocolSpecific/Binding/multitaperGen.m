function varargout = multitaperGen(x,y, varargin);
...
Cross_Spectra = zeros(params.nfft, size(x,1)*size(x,1));
Auto_Spectra1 = zeros(params.nfft, size(x,1)*size(x,1));
Auto_Spectra2 = zeros(params.nfft, size(x,1)*size(x,1));
...
for i = 1:size(x,1)
    for j = 1:size(x,1)      
        [Cross_Spectra(:,count),k] = mtm_spectrum(x(i,:),y(j,:), params);
        [Auto_Spectra1(:,count),k] = mtm_spectrum(x(i,:),x(i,:), params);
        [Auto_Spectra2(:,count),k] = mtm_spectrum(y(j,:),y(j,:), params);
        count = count + 1;
    end
end    
...
Sxy_unscaled = Cross_Spectra; % Make sure Sxx_unscale, Sxx and Pxx are all columns
Sxx_unscaled = Auto_Spectra1; % Make sure Sxx_unscale, Sxx and Pxx are all columns
Syy_unscaled = Auto_Spectra2; % Make sure Sxx_unscale, Sxx and Pxx are all columns
...
if strcmp(params.range,'onesided'),
   ...
   Sxx_unscaled = Auto_Spectra1(select,:); % Take only [0,pi] or [0,pi)
   Syy_unscaled = Auto_Spectra2(select,:); % Take only [0,pi] or [0,pi)
   Sxy_unscaled = Cross_Spectra(select,:); % Take only [0,pi] or [0,pi)
   f = w(select);
...
if N<=nfft
    ...
    %use this line for non-zero mean spike trains
...
case 'adapt'
   ...
   % Do the iteration:
   while ( sum(abs(S-S1)/nfft)>tol ) && (i < 10)
      i=i+1;
      % calculate weights
...
if length(varargin) > 2, 
   [params,err_msg] = psdoptions(isreal(x),params,varargin{3:end});
