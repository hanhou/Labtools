function varargout = multitaper2(x,y, varargin);
%Multitaper   Power Spectrum estimate via the Thomson multitaper 
% contains correction for spike trains
%   method (MTM).
%
...
if strcmp(params.range,'onesided'),
   ...
   Syy = [Syy_unscaled(1); 2*Syy_unscaled(2:end-1); Syy_unscaled(end)]; % This is the one-sided spectrum [Power]
   Sxy = [Sxy_unscaled(1); 2*Sxy_unscaled(2:end-1); Sxy_unscaled(end)]; % This is the one-sided spectrum [Power]
end
...
%load indices for spike and LFP db
TEMPO_Defs;

% Compute the windowed DFTs.
if N<=nfft
    % for each stream of data, consider if LFPs or spike trains.  If spike
    % trains, then subtract the mean * H.  
    switch params.x_neural_db + 10*params.y_neural_db
        % correction for pt process not being mean 0 signal:  - mean(x).*H(x);
        case SPIKE_DB + 10*SPIKE_DB %(both spike DB)
            Sk=(fft(E(:,1:k).*x(:,ones(1,k)),nfft) - mean(x).*H ) .* (conj(fft(E(:,1:k).*y(:,ones(1,k)),nfft)  - mean(y).*H )   );
        case SPIKE_DB + 10*LFP_DB %hybrid - x -SPIKE, y - LFP
            Sk=(fft(E(:,1:k).*x(:,ones(1,k)),nfft) - mean(x).*H ) .* (conj(fft(E(:,1:k).*y(:,ones(1,k)),nfft)  )   );
        case LFP_DB + 10*SPIKE_DB %hybrid - x -LFP, y - SPIKE
            Sk=(fft(E(:,1:k).*x(:,ones(1,k)),nfft)  ) .* (conj(fft(E(:,1:k).*y(:,ones(1,k)),nfft)  - mean(y).*H )   );
        case LFP_DB + 10*LFP_DB %2 LFP DB
            Sk=(fft(E(:,1:k).*x(:,ones(1,k)),nfft) .* conj(fft(E(:,1:k).*y(:,ones(1,k)),nfft) )  );
    end

else  % Wrap the data modulo nfft if N > nfft
    switch x_neural_db + 10*y_neural_db
        ...
        case SPIKE_DB + 10*LFP_DB %hybrid - x -SPIKE, y - LFP
            Sk=(czt(E(:,1:k).*x(:,ones(1,k)),nfft)  - mean(x).*H ).* (conj(czt(E(:,1:k).*y(:,ones(1,k)),nfft) ) ); 
        case LFP_DB + 10*SPIKE_DB %hybrid - x -LFP, y - SPIKE
            Sk=(czt(E(:,1:k).*x(:,ones(1,k)),nfft)  ).* (conj(czt(E(:,1:k).*y(:,ones(1,k)),nfft)  - mean(y).*H ) ); 
        case LFP_DB + 10*LFP_DB %2 LFP DB
...
case 'adapt'
   ...
   % Do the iteration:
   while ( sum(abs(S-S1)/nfft)>tol ) && (i < 50)
      i=i+1;
      % calculate weights
...
if length(varargin) > 2, 
    [params,err_msg] = psdoptions(isreal(x),params,varargin{indx:6});
    params.x_neural_db = varargin{7};
    params.y_neural_db = varargin{8};
    if err_msg, return, end;
end
