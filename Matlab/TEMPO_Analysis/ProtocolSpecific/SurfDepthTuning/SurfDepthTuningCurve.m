%-----------------------------------------------------------------------------------------------------------------------
%-- SurfDepthTuningCurve.m -- Plots a horizontal disparity tuning curve, possibly for multiple speeds, and including
%--	monoc and uncorrelated control conditions.
%--	GCD, 1/26/00
%-----------------------------------------------------------------------------------------------------------------------
function SurfDepthTuningCurve(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
depth = data.dots_params(DEPTH_DIST_SIM,:,PATCH1);
%hor_disp'

%get indices of any NULL conditions (for measuring spontaneous activity)
null_trials = logical( (depth == data.one_time_params(NULL_VALUE)) );
...
control_trials = logical( (depth == LEYE_CONTROL) | (depth == REYE_CONTROL) | (depth == UNCORR_CONTROL) );

unique_depth = munique(depth(~null_trials & ~control_trials)')

%now, remove trials from depth and spike_rates that do not fall between BegTrial and EndTrial
trials = 1:length(depth);		% a vector of trial indices
select_trials = ( (trials >= BegTrial) & (trials <= EndTrial) );

%calculate the average intra-trial variance in vergence angle, added by GCD 9/26/02
verg_trial_std = [];
eye_traces = ComputeAllEyeTraces(data, length(depth), StartCode, StopCode, 0, 0);
for i = 1:length(depth)  %loop through all trials
...
figure(2);
set(gcf,'PaperPosition', [.2 .2 8 10.7], 'Position', [450 50 500 573], 'Name', 'Depth Tuning Curve');
subplot(2, 1, 2);
...
for i=1:length(unique_speed)	%for each different speed value, plot a separate disparity tuning curve
    ...
    plot_x = depth(speed_select & ~null_trials & ~control_trials & select_trials);
    ...
    Leye_trials = logical( (depth == LEYE_CONTROL) );
    ...
    Reye_trials = logical( (depth == REYE_CONTROL) );
    ...
    Uncorr_trials = logical( (depth == UNCORR_CONTROL) );
...
%    fclose(fwriteid);
    ...
    %outfile = ['Z:\Users\jerry\GradAnalysis\depthTuningSummary_8.27.02.dat'];
    outfile = [PATHOUT 'depthTuningSummary.dat'];
    printflag = 0;
    ...
    for j = 1:length(unique_speed)
        buff = sprintf('%s\t %6.1f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.2f\t %5.2f\t %6.3f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.2f\t %6.3f\t %6.2f\t %6.2f\t %6.2f\t %6.3f\t %10.8f\t %6.3f\t %6.3f\t %5.3f\t %10.3f\t %10.3f\t %10.8f\t %10.8f\t %10.8f\t %6.4f\t %6.4f\t %6.4f\t %10.8f\t %10.8f\t', ...
            FILE, data.neuron_params(PREFERRED_DIRECTION, 1), data.neuron_params(PREFERRED_SPEED, 1), data.neuron_params(PREFERRED_DEPTH, 1), data.neuron_params(RF_XCTR, 1), data.neuron_params(RF_YCTR, 1), data.neuron_params(RF_DIAMETER, 1),...
            unique_speed(j), pmax(j).x, pmax(j).y, pmin(j).x, pmin(j).y, avg_resp(j), null_rate, DMI(j), cont_rate(j).left, cont_rate(j).right, cont_rate(j).uncorr, DTI(j), p_value(j), DDI(j), var_term(j), ROC_value(j), MSgroup(j), MSerror(j), AncPdisp(j), AncPverg(j), AncPinter(j), verg_sd(j), max(unique_depth), min(unique_depth), avg_intratrial_verg_std, avg_intertrial_verg_std );
        fprintf(fid, '%s', buff);
        fprintf(fid, '\r\n');
    end
    fclose(fid);
    ...
    if (grad_print == 1)
        ...
        outfile = [PATHOUT 'depth_metrics.dat'];
        printflag = 0;
        if (exist(outfile, 'file') == 0)    %file does not yet exist
            printflag = 1;
        end
