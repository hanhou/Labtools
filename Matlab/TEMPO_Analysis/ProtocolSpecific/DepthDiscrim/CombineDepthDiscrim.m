%-----------------------------------------------------------------------------------------------------------------------
%-- CombineDepthDiscrim.m -- Does not plot anything but instead saves the
...
%Critically, there must exist a file DepthDiscrim\CumulativeFiles\DepthDataSort.txt
...
%--	VR, 6/15/06
%-----------------------------------------------------------------------------------------------------------------------
function [monkey_alpha] = CombineDepthDiscrim(data, Protocol, Analysis, SpikeChan, StartCode, StopCode, BegTrial, EndTrial, StartOffset, StopOffset, PATH, FILE);
...
%get the binocular correlatiosn
...
for j = 1:length(unique_x_ctr)
    for i=1:length(unique_bin_corr)
        trial_inds = ((x_ctr == unique_x_ctr(j)) & (binoc_corr == unique_bin_corr(i)) & select_trials);
...
infile = [BASE_PATH 'ProtocolSpecific\DepthDiscrim\CumulativeFiles\DepthDataSort.txt'];
[AllFiles,AllInjections,AllDays] = textread(infile,'%s%d%d');
file_ind = find(strcmp(AllFiles,FILE),1);
if (length(file_ind) == 0)    
    disp('File Not Found in DepthDataSort.txt');
    return
end
...
outfile = sprintf('%sProtocolSpecific\\DepthDiscrim\\CumulativeFiles\\Depth_Inj%d_Day%d.mat',BASE_PATH,inj,day);
if (exist(outfile, 'file') == 0)
    ...
    save(outfile, 'unique_x_ctr', 'unique_bin_corr', 'cum_corr_trials', 'cum_total_trials');
else
    load(outfile);
...
for j = 1:length(unique_x_ctr)
    for i=1:length(unique_bin_corr)
        ...
        fit_data(j,i,1) = unique_bin_corr(i);
...
if (output)
    ...
    outfile = [BASE_PATH 'ProtocolSpecific\DepthDiscrim\Combined_psycho_Curve_summary.dat'];
    printflag = 0;
    ...
    fid = fopen(outfile, 'a');
    if (printflag)
        fprintf(fid, 'FILE\t Inj\t Day\t PrDir\t PrSpd\t PrHDsp\t RFX\t RFY\t RFDiam\t MThr\t MSlp\t Ntrials\t BinCorr1\t Pct1\t BinCorr2\t Pct2\t BinCorr3\t Pct3\t BinCorr4\t Pct4\t BinCorr5\t Pct5\t BinCorr6\t Pct6\t ');
        fprintf(fid, '\r\n');
        printflag = 0;
    end
    ...
    for j = 1:length(unique_x_ctr)
        ...
        for i = 1:length(unique_bin_corr)
            buff = [buff sprintf('%6.2f\t %6.3f\t ',unique_bin_corr(i),pct_correct(j,i))];
        end
        fprintf(fid, '%s', buff);
        fprintf(fid, '\r\n');
    end
